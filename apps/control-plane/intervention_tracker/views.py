...
from .serializers import (
    ...
    InterventionRuleSerializer,
    InterventionWorkflowSerializer,
    InterventionStepSerializer,
    InterventionOutcomeSerializer,
    AuditLogSerializer,
    CreateInterventionRequestSerializer,
    EvaluateRulesRequestSerializer,
    UpdateStepRequestSerializer,
...
class InterventionViewSet(viewsets.ModelViewSet):
    ...
    def create_intervention(self, request, tenant_slug=None):
        ...
        return Response(
            InterventionSerializer(intervention).data, status=status.HTTP_201_CREATED
        )
    ...
    def update_step(self, request, pk=None, tenant_slug=None):
        ...
        step = InterventionStep.objects.get(
            id=data["step_id"], intervention=intervention
        )
    ...
    def record_outcome(self, request, pk=None, tenant_slug=None):
        ...
        if outcome.target_achieved:
            ...
            and outcome.baseline_value
            and outcome.actual_value > outcome.baseline_value
        ):
    ...
    def dashboard(self, request, tenant_slug=None):
        ...
        active_interventions = queryset.filter(
        ...
        success_rate = (
            (successful / completed.count() * 100) if completed.count() > 0 else 0
        )
        ...
        followup_required = queryset.filter(
            requires_followup=True, status__in=["completed", "in_progress"]
        ).count()
    ...
    def audit_report(self, request, tenant_slug=None):
        ...
        report = {
            ...
            "summary": {
                "total_interventions": queryset.count(),
                "by_type": list(
                    queryset.values("intervention_type").annotate(count=Count("id"))
                ),
                "by_outcome": list(
                    queryset.values("outcome_achieved").annotate(count=Count("id"))
                ),
                "compliance_categories": list(
                    queryset.exclude(compliance_category="")
                    .values("compliance_category")
                    .annotate(count=Count("id"))
                ),
            },
...
class InterventionRuleViewSet(viewsets.ModelViewSet):
    ...
    def evaluate_rules(self, request, tenant_slug=None):
        ...
        rules = InterventionRule.objects.filter(
            tenant=tenant_slug, is_active=True
        ).order_by("-priority")
