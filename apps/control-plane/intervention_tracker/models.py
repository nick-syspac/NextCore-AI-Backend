...
class Intervention(models.Model):
    ...
    trigger_rule_id = models.CharField(max_length=100, blank=True)  # Which rule triggered it
    ...
    outcome_evidence = models.JSONField(default=list)  # Links to documents, photos, etc.
    ...
    # Referrals
    referred_to = models.CharField(max_length=200, blank=True)  # e.g., Counseling, Disability Services
    ...
    compliance_category = models.CharField(max_length=100, blank=True)  # e.g., "ASQA Requirement 1.5"
...
class InterventionRule(models.Model):
    ...
    # Action to take
    intervention_type = models.CharField(max_length=50)  # Maps to Intervention.intervention_type
    priority_level = models.CharField(max_length=20, default='medium')
    assigned_to_role = models.CharField(max_length=100, blank=True)  # Auto-assign to this role
    
    # Notification settings
    notify_staff = models.BooleanField(default=True)
    notification_recipients = models.JSONField(default=list)  # Email addresses or user IDs
...
class InterventionWorkflow(models.Model):
    ...
    description = models.TextField()
    intervention_types = models.JSONField(default=list)  # Which intervention types use this workflow
    is_active = models.BooleanField(default=True)
...
class InterventionStep(models.Model):
    ...
    intervention = models.ForeignKey(Intervention, on_delete=models.CASCADE, related_name='workflow_steps')
    workflow = models.ForeignKey(InterventionWorkflow, on_delete=models.SET_NULL, null=True, blank=True)
...
class InterventionOutcome(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.baseline_value is not None and self.actual_value is not None and self.baseline_value != 0:
            self.improvement_percentage = ((self.actual_value - self.baseline_value) / self.baseline_value) * 100
...
class AuditLog(models.Model):
    ...
    tenant = models.CharField(max_length=100, db_index=True)
    intervention = models.ForeignKey(Intervention, on_delete=models.CASCADE, related_name='audit_logs')
