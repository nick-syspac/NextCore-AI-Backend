...
from .models import (
    ...
    CoachingInsight, CoachConfiguration
)
from .serializers import (
    ChatSessionSerializer, ChatSessionListSerializer, ChatMessageSerializer,
    KnowledgeDocumentSerializer, CoachingInsightSerializer,
    CoachConfigurationSerializer, SendMessageRequestSerializer,
...
class ChatSessionViewSet(viewsets.ModelViewSet):
    ...
    def dashboard(self, request, tenant_slug=None):
        ...
        avg_satisfaction = queryset.filter(
            satisfaction_rating__isnull=False
        ).aggregate(avg=Avg('satisfaction_rating'))['avg'] or 0.0
        
        total_messages = queryset.aggregate(
            total=models.Sum('message_count')
        )['total'] or 0
        
        # Session type breakdown
        session_types = queryset.values('session_type').annotate(
            count=Count('id')
        ).order_by('-count')
    ...
    def _analyze_sentiment(self, message):
        ...
        negative_words = ['confused', 'stuck', 'hard', 'difficult', 'frustrated', 
                          'don\'t understand', 'help', 'struggling']
        positive_words = ['thanks', 'thank you', 'great', 'helpful', 'understand',
                          'clear', 'got it', 'makes sense']
    ...
    def _generate_coach_response(self, tenant, session, message, context):
        ...
        if any(word in message_lower for word in ['hello', 'hi', 'hey']):
        ...
        elif 'help' in message_lower and any(word in message_lower for word in ['homework', 'assignment']):
            ...
            concept = message.split('explain')[-1].strip() if 'explain' in message_lower else 'this concept'
            response = f"Great question! Let me explain {concept}. " + \
            ...
            response = "Let's prepare you for success on your exam! I can help with:\n" + \
                      "• Reviewing key concepts\n• Practice problems\n• Study strategies\n• Time management\n\nWhat area would you like to focus on first?"
        elif any(word in message_lower for word in ['stuck', 'confused', 'don\'t understand']):
            response = "No worries - that's what I'm here for! Let's break this down into smaller pieces. " + \
                      "Can you tell me which specific part is confusing? Sometimes a different explanation or example can make it click."
        elif any(word in message_lower for word in ['thanks', 'thank you']):
            response = "You're very welcome! I'm glad I could help. Remember, I'm here 24/7 whenever you need support. " + \
                      "Is there anything else you'd like to work on?"
        else:
            response = f"I understand you're asking about this topic. Based on the course materials I have access to, " + \
...
class CoachingInsightViewSet(viewsets.ModelViewSet):
    ...
    def generate_insights(self, request, tenant_slug=None):
        ...
        total_messages = sessions.aggregate(total=models.Sum('message_count'))['total'] or 0
        total_duration = sessions.aggregate(total=models.Sum('total_duration_minutes'))['total'] or 0
        ...
        for session in sessions:
            session_type_dist[session.session_type] = session_type_dist.get(session.session_type, 0) + 1
        ...
        most_discussed_subjects = [item for item, count in Counter(subjects).most_common(5)]
        ...
        avg_satisfaction = rated_sessions.aggregate(avg=Avg('satisfaction_rating'))['avg'] or 0.0
