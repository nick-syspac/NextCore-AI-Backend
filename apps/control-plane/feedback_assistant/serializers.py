...
class FeedbackCriterionSerializer(serializers.ModelSerializer):
    rubric_criterion_name = serializers.CharField(
        source="rubric_criterion.title", read_only=True
    )
...
class BatchGenerateFeedbackRequestSerializer(serializers.Serializer):
    ...
    feedbacks = serializers.ListField(
        child=serializers.DictField(), help_text="List of feedback generation requests"
    )
    ...
    def validate_feedbacks(self, value):
        ...
        if not value:
            raise serializers.ValidationError(
                "At least one feedback request is required"
            )

        for idx, feedback in enumerate(value):
            if "student_id" not in feedback:
                raise serializers.ValidationError(
                    f"Feedback {idx}: student_id is required"
                )
            if "student_name" not in feedback:
                raise serializers.ValidationError(
                    f"Feedback {idx}: student_name is required"
                )
