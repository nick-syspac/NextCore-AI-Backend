...
class FeedbackTemplate(models.Model):
    ...
    feedback_type = models.CharField(
        max_length=50, choices=FEEDBACK_TYPE_CHOICES, default="formative"
    )
    sentiment = models.CharField(
        max_length=50, choices=SENTIMENT_CHOICES, default="constructive"
    )
    tone = models.CharField(max_length=50, choices=TONE_CHOICES, default="professional")

    # Rubric mapping
    rubric = models.ForeignKey(
        "rubric_generator.Rubric", on_delete=models.SET_NULL, null=True, blank=True
    )
    maps_to_criteria = models.JSONField(
        default=list, blank=True, help_text="List of rubric criterion IDs"
    )
    ...
    opening_template = models.TextField(
        default="Dear {student_name},", help_text="Opening statement template"
    )
    strengths_template = models.TextField(
        default="Your strengths include: {strengths}", blank=True
    )
    improvements_template = models.TextField(
        default="Areas for improvement: {improvements}", blank=True
    )
    next_steps_template = models.TextField(
        default="Next steps: {next_steps}", blank=True
    )
    ...
    average_generation_time = models.FloatField(
        default=0.0, help_text="Average time in seconds"
    )
...
class GeneratedFeedback(models.Model):
    ...
    template = models.ForeignKey(
        FeedbackTemplate, on_delete=models.CASCADE, related_name="generated_feedbacks"
    )
    ...
    rubric_scores = models.JSONField(
        default=dict, blank=True, help_text="Scores per rubric criterion"
    )
    ...
    reading_level = models.CharField(
        max_length=50, blank=True, help_text="e.g., Grade 10, University"
    )
    ...
    delivery_method = models.CharField(
        max_length=50, blank=True, help_text="email, LMS, portal, etc."
    )
    ...
    generation_time = models.FloatField(
        default=0.0, help_text="Time to generate in seconds"
    )
...
class FeedbackCriterion(models.Model):
    ...
    template = models.ForeignKey(
        FeedbackTemplate, on_delete=models.CASCADE, related_name="criteria"
    )
    ...
    excellent_feedback = models.TextField(
        help_text="Feedback for excellent performance"
    )
    good_feedback = models.TextField(help_text="Feedback for good performance")
    satisfactory_feedback = models.TextField(
        help_text="Feedback for satisfactory performance"
    )
    needs_improvement_feedback = models.TextField(
        help_text="Feedback for needs improvement"
    )
...
class FeedbackLog(models.Model):
    ...
    # Reference
    template = models.ForeignKey(
        FeedbackTemplate, on_delete=models.CASCADE, related_name="logs"
    )
