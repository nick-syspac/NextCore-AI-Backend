...
from .serializers import (
    ...
    FeedbackTemplateDetailSerializer,
    GeneratedFeedbackListSerializer,
    GeneratedFeedbackDetailSerializer,
    FeedbackCriterionSerializer,
    FeedbackLogSerializer,
    GenerateFeedbackRequestSerializer,
    BatchGenerateFeedbackRequestSerializer,
)
...
class FeedbackTemplateViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related("created_by", "rubric").prefetch_related(
            "criteria", "generated_feedbacks"
        )
    ...
    def generate_feedback(self, request, tenant_slug=None, pk=None):
        ...
        if template.total_feedback_generated > 0:
            ...
            template.average_generation_time = (
                total_time / template.total_feedback_generated
            )
        template.save()
    ...
    def generate_batch(self, request, tenant_slug=None, pk=None):
        ...
        if template.total_feedback_generated > 0:
            ...
            template.average_generation_time = (
                total_generation_time / template.total_feedback_generated
            )
        template.save()

        # Calculate average metrics
        avg_sentiment = sum(f.sentiment_score for f in generated_feedbacks) / len(
            generated_feedbacks
        )
        avg_personalization = sum(
            f.personalization_score for f in generated_feedbacks
        ) / len(generated_feedbacks)
        ...
        response_serializer = GeneratedFeedbackListSerializer(
            generated_feedbacks, many=True
        )
        ...
        return Response(
            {
                ...
                "total_time": total_time,
                "average_time": (
                    total_time / len(generated_feedbacks) if generated_feedbacks else 0
                ),
    ...
    def _generate_feedback(self, template, feedback_data):
        ...
        if template.include_strengths and strengths:
            strengths_text = template.strengths_template.replace(
                "{strengths}", self._format_list(strengths, template.positivity_level)
            )
        ...
        if template.include_next_steps and next_steps:
            next_steps_text = template.next_steps_template.replace(
                "{next_steps}", self._format_list(next_steps, template.positivity_level)
            )
            feedback_parts.append(f"\n\n{next_steps_text}")

        # Closing
        if template.include_encouragement and template.closing_template:
            closing = self._personalize_closing(
                template.closing_template, template.positivity_level
            )
            feedback_parts.append(f"\n\n{closing}")
        ...
        sentiment_score = (
            template.positivity_level - 5.5
        ) / 4.5  # Maps 1-10 to roughly -1 to +1
    ...
    def _generate_improvements(self, template, percentage, rubric_scores):
        ...
        improvements = random.sample(
            improvement_pool, min(num_improvements, len(improvement_pool))
        )
    ...
    def _personalize_closing(self, closing, positivity_level):
        ...
        return (
            random.choice(closings) if closing == "Keep up the great work!" else closing
        )
