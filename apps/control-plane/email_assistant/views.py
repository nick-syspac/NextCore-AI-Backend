...
class StudentMessageViewSet(viewsets.ModelViewSet):
    ...
    def generate_reply(self, request):
        ...
        response_serializer = GenerateReplyResponseSerializer(
            data={
                "draft_id": draft.id,
    ...
    def refine_tone(self, request):
        ...
        response_serializer = RefineToneResponseSerializer(
            data={
                "draft_id": draft.id,
    ...
    def save_template(self, request):
        ...
        response_serializer = SaveTemplateResponseSerializer(
            data={
    ...
    def suggest_replies(self, request):
        ...
        response_serializer = SuggestRepliesResponseSerializer(
            data={
    ...
    def analyze_sentiment(self, request):
        ...
        if (
            "technical" in message_lower
            or "login" in message_lower
            or "access" in message_lower
        ):
        ...
        response_serializer = AnalyzeSentimentResponseSerializer(
            data={
                "message_id": message.id,
                "sentiment": sentiment,
    ...
    def send_reply(self, request):
        ...
        time_to_first_draft = (
            draft.generated_at - message.received_date
        ).total_seconds()
        ...
        response_serializer = SendReplyResponseSerializer(
            data={
    ...
    def dashboard(self, request):
        ...
        total_time_saved = (
            history.aggregate(total=Sum("time_saved_seconds"))["total"] or 0
        )
        total_time_saved_hours = total_time_saved / 3600
        avg_time_saved = history.aggregate(avg=Avg("time_saved_seconds"))["avg"] or 0
        avg_time_saved_pct = (
            history.aggregate(avg=Avg("time_saved_percentage"))["avg"] or 0
        )
        ...
        dashboard_data = {
            ...
            "recent_messages": StudentMessageListSerializer(
                recent_messages, many=True
            ).data,
            "recent_drafts": DraftReplyListSerializer(recent_drafts, many=True).data,
            "top_templates": MessageTemplateListSerializer(
                top_templates, many=True
            ).data,
        }
