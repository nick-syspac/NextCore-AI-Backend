...
class StudentMessage(models.Model):
    ...
    # Message details
    message_type = models.CharField(max_length=20, choices=MESSAGE_TYPES, default='email')
    ...
    category = models.CharField(max_length=100, blank=True)  # e.g., "Assessment Query", "Enrollment", "Technical Support"
    detected_sentiment = models.CharField(max_length=50, blank=True)  # positive, negative, neutral, frustrated
    detected_topics = models.JSONField(default=list, blank=True)  # ["assessment", "deadline", "extension"]
    ...
    # Context
    conversation_thread = models.ForeignKey('ConversationThread', on_delete=models.SET_NULL, null=True, blank=True, related_name='messages')
...
class DraftReply(models.Model):
    ...
    student_message = models.ForeignKey(StudentMessage, on_delete=models.CASCADE, related_name='drafts')
    ...
    # Generation settings
    tone_used = models.CharField(max_length=50)  # professional, friendly, empathetic, formal, casual
    ...
    # Generation metadata
    generation_status = models.CharField(max_length=20, choices=GENERATION_STATUS, default='completed')
    ...
    def save(self, *args, **kwargs):
        ...
        if self.reply_body:
            ...
            self.estimated_reading_time_seconds = int((self.word_count / 200) * 60)  # 200 words per minute
        super().save(*args, **kwargs)
...
class MessageTemplate(models.Model):
    ...
    placeholders = models.JSONField(default=list, blank=True)  # ["{student_name}", "{due_date}", "{unit_code}"]
    ...
    success_rate = models.FloatField(default=0.0)  # % of times used template was sent without major edits
...
class ToneProfile(models.Model):
    ...
    # Tone settings
    tone_descriptor = models.CharField(max_length=50)  # professional, friendly, empathetic, formal
    formality_level = models.IntegerField(default=3)  # 1 (very casual) to 5 (very formal)
    empathy_level = models.IntegerField(default=3)  # 1 (neutral) to 5 (highly empathetic)
    ...
    # Use cases
    recommended_for = models.JSONField(default=list, blank=True)  # ["complaints", "technical_support"]
...
class ReplyHistory(models.Model):
    ...
    # References
    student_message = models.ForeignKey(StudentMessage, on_delete=models.CASCADE, related_name='reply_history')
    draft_reply = models.ForeignKey(DraftReply, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    # Timing metrics
    time_to_first_draft_seconds = models.IntegerField(default=0)  # From message received to first draft
    time_to_send_seconds = models.IntegerField(default=0)  # From message received to sent
    edit_count = models.IntegerField(default=0)  # How many times draft was edited
    
    # Efficiency metrics
    estimated_manual_time_seconds = models.IntegerField(default=300)  # Estimated time if done manually (5 min default)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.time_to_send_seconds > 0 and self.estimated_manual_time_seconds > 0:
            self.time_saved_seconds = self.estimated_manual_time_seconds - self.time_to_send_seconds
            if self.estimated_manual_time_seconds > 0:
                self.time_saved_percentage = (self.time_saved_seconds / self.estimated_manual_time_seconds) * 100
