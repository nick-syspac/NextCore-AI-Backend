...
class StudentMessageListSerializer(serializers.ModelSerializer):
    class Meta:
        model = StudentMessage
        fields = ['id', 'message_number', 'student_name', 'subject', 'message_type', 
                  'priority', 'status', 'received_date', 'detected_sentiment']


class DraftReplySerializer(serializers.ModelSerializer):
    student_message_details = StudentMessageListSerializer(source='student_message', read_only=True)
...
class DraftReplyListSerializer(serializers.ModelSerializer):
    class Meta:
        model = DraftReply
        fields = ['id', 'draft_number', 'tone_used', 'confidence_score', 
                  'was_sent', 'generated_at', 'word_count']
...
class MessageTemplateListSerializer(serializers.ModelSerializer):
    class Meta:
        model = MessageTemplate
        fields = ['id', 'template_number', 'name', 'template_type', 
...
class RefineToneRequestSerializer(serializers.Serializer):
    ...
    formality_adjustment = serializers.IntegerField(required=False, min_value=-2, max_value=2)
...
class SaveTemplateRequestSerializer(serializers.Serializer):
    ...
    template_type = serializers.ChoiceField(
        choices=['assessment', 'enrollment', 'technical', 'extension', 'general', 'complaint', 'feedback']
    )
...
class SuggestRepliesRequestSerializer(serializers.Serializer):
    ...
    variety = serializers.BooleanField(default=True)  # Generate replies with different tones
