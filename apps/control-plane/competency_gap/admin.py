...
class TrainerQualificationAdmin(admin.ModelAdmin):
    list_display = ['qualification_id', 'trainer_name', 'qualification_code', 
                    'qualification_type', 'verification_status', 'date_obtained', 'expiry_date']
    list_filter = ['verification_status', 'qualification_type', 'recent_industry_work']
    search_fields = ['trainer_name', 'trainer_id', 'qualification_code', 'qualification_name']
    ...
    fieldsets = (
        ...
        ('Qualification Details', {
            'fields': ('qualification_type', 'qualification_code', 'qualification_name', 
                      'issuing_organization')
        }),
        ('Verification', {
            'fields': ('date_obtained', 'expiry_date', 'verification_status', 
                      'verification_document')
        }),
        ('Competencies', {
            'fields': ('competency_areas', 'units_covered')
        }),
        ('Industry Currency', {
...
class UnitOfCompetencyAdmin(admin.ModelAdmin):
    list_display = ['unit_id', 'unit_code', 'unit_name', 'unit_type', 
                    'qualification_code', 'requires_tae', 'requires_industry_currency']
    ...
    fieldsets = (
        ('Unit Information', {
            'fields': ('unit_id', 'tenant', 'unit_code', 'unit_name', 'unit_type', 
                      'qualification_code')
        }),
        ('Requirements', {
            'fields': ('required_qualifications', 'required_competency_areas', 
                      'required_industry_experience', 'requires_tae', 'requires_industry_currency')
        }),
        ('Content', {
            'fields': ('learning_outcomes', 'assessment_methods', 'technical_skills')
        }),
        ('Graph Relationships', {
            'fields': ('prerequisite_units', 'related_units')
        }),
        ('Metadata', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
...
class TrainerAssignmentAdmin(admin.ModelAdmin):
    list_display = ['assignment_id', 'trainer_name', 'unit', 'assignment_status', 
                    'meets_requirements', 'compliance_score', 'assigned_date']
    ...
    fieldsets = (
        ('Assignment Information', {
            'fields': ('assignment_id', 'tenant', 'trainer_id', 'trainer_name', 'unit')
        }),
        ('Status', {
            'fields': ('assignment_status', 'assigned_date', 'approved_by', 'approved_date')
        }),
        ('Compliance', {
            'fields': ('meets_requirements', 'compliance_score', 'gaps_identified', 
...
class CompetencyGapAdmin(admin.ModelAdmin):
    list_display = ['gap_id', 'trainer_name', 'unit', 'gap_type', 'gap_severity', 
    ...
    fieldsets = (
        ('Gap Information', {
            'fields': ('gap_id', 'tenant', 'trainer_id', 'trainer_name', 'unit', 'assignment')
        }),
        ('Gap Details', {
            'fields': ('gap_type', 'gap_severity', 'gap_description')
        }),
        ('Requirements', {
            'fields': ('required_qualification', 'required_competency', 
                      'required_experience_years', 'current_qualifications')
        }),
        ('Resolution', {
...
class QualificationMappingAdmin(admin.ModelAdmin):
    list_display = ['mapping_id', 'source_qualification_code', 'match_strength', 
                    'match_confidence', 'verified', 'mapping_source']
    ...
    fieldsets = (
        ('Mapping Information', {
            'fields': ('mapping_id', 'tenant', 'source_qualification_code', 
        ...
        ('Units Coverage', {
            'fields': ('units_covered', 'units_partially_covered')
        }),
        ('Metadata', {
...
class ComplianceCheckAdmin(admin.ModelAdmin):
    list_display = ['check_id', 'check_type', 'check_status', 'total_assignments_checked', 
                    'overall_compliance_score', 'gaps_found', 'created_at']
    ...
    fieldsets = (
        ...
        ('Scope', {
            'fields': ('trainer_ids', 'unit_codes')
        }),
        ('Results Summary', {
            'fields': ('total_assignments_checked', 'compliant_assignments', 
                      'non_compliant_assignments', 'gaps_found')
        }),
        ('Severity Breakdown', {
        ...
        ('Execution', {
            'fields': ('started_at', 'completed_at', 'execution_time_seconds', 'error_message')
        }),
        ('Metadata', {
            'fields': ('created_at', 'updated_at'),
            'classes': ('collapse',)
