...
class TrainerQualification(models.Model):
    ...
    verification_document = models.CharField(max_length=500, blank=True)  # File path or URL
...
class UnitOfCompetency(models.Model):
    ...
    required_qualifications = models.JSONField(default=list)  # List of required qualification codes
    required_competency_areas = models.JSONField(default=list)  # List of competency areas
    ...
    # Graph relationships
    prerequisite_units = models.JSONField(default=list)  # Unit codes that are prerequisites
...
class TrainerAssignment(models.Model):
    ...
    trainer_id = models.CharField(max_length=100)
    trainer_name = models.CharField(max_length=200)
    unit = models.ForeignKey(UnitOfCompetency, on_delete=models.CASCADE, related_name='assignments')
    
    # Assignment details
    assignment_status = models.CharField(max_length=20, choices=ASSIGNMENT_STATUS, default='pending')
    ...
    matching_qualifications = models.JSONField(default=list)  # List of qualification IDs
...
class CompetencyGap(models.Model):
    ...
    trainer_id = models.CharField(max_length=100)
    trainer_name = models.CharField(max_length=200)
    unit = models.ForeignKey(UnitOfCompetency, on_delete=models.CASCADE, related_name='gaps')
    assignment = models.ForeignKey(TrainerAssignment, on_delete=models.CASCADE, related_name='gaps', null=True, blank=True)
    ...
    # Current state
    current_qualifications = models.JSONField(default=list)  # What trainer currently has
    ...
    estimated_resolution_time = models.CharField(max_length=100, blank=True)  # e.g., "6 months"
...
class QualificationMapping(models.Model):
    ...
    equivalent_qualifications = models.JSONField(default=list)  # List of equivalent qualification codes
    superseded_by = models.CharField(max_length=50, blank=True)
    supersedes = models.CharField(max_length=50, blank=True)
    
    # Units coverage
    units_covered = models.JSONField(default=list)  # Unit codes covered by this qualification
    units_partially_covered = models.JSONField(default=list)  # Partially covered units
    
    # Metadata
    mapping_source = models.CharField(max_length=100, default='manual')  # manual, training.gov.au, ml_model
...
class ComplianceCheck(models.Model):
    ...
    # Check details
    check_type = models.CharField(max_length=50, default='full_matrix')  # full_matrix, trainer, unit, qualification
    check_status = models.CharField(max_length=20, choices=CHECK_STATUS, default='pending')
    
    # Scope
    trainer_ids = models.JSONField(default=list)  # List of trainer IDs to check (empty = all)
    unit_codes = models.JSONField(default=list)  # List of unit codes to check (empty = all)
    ...
    detailed_results = models.JSONField(default=dict)  # Detailed breakdown by trainer/unit
