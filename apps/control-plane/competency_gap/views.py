...
from .serializers import (
    TrainerQualificationSerializer, TrainerQualificationListSerializer,
    UnitOfCompetencySerializer, TrainerAssignmentSerializer,
    CompetencyGapSerializer, QualificationMappingSerializer,
    ComplianceCheckSerializer, CheckGapsRequestSerializer,
    CheckGapsResponseSerializer, AssignTrainerRequestSerializer,
    AssignTrainerResponseSerializer, ValidateMatrixRequestSerializer,
    ValidateMatrixResponseSerializer, GraphAnalysisRequestSerializer,
    GraphAnalysisResponseSerializer, GenerateComplianceReportRequestSerializer,
    GenerateComplianceReportResponseSerializer, DashboardStatsSerializer,
    BulkAssignRequestSerializer, BulkAssignResponseSerializer
)
...
class TrainerQualificationViewSet(viewsets.ModelViewSet):
    ...
    def check_gaps(self, request):
        ...
        trainer_qual_codes = list(qualifications.values_list('qualification_code', flat=True))
        ...
        max_experience = qualifications.aggregate(Avg('industry_experience_years'))['industry_experience_years__avg'] or 0
        ...
        meets_requirements = len(gaps_found) == 0 or all(g['severity'] not in ['critical'] for g in gaps_found)
        can_deliver = meets_requirements and compliance_score >= 60
        
        recommendations = [g['recommendation'] for g in gaps_found if data.get('include_recommendations', True)]
    ...
    def assign_trainer(self, request):
        ...
        assignment = TrainerAssignment.objects.create(
            ...
            matching_qualifications=[],
            assignment_status='approved' if (meets_requirements or data.get('override_gaps', False)) else 'under_review',
    ...
    def bulk_assign(self, request):
        ...
        for unit_id in unit_ids:
            ...
            response = self.assign_trainer(type('Request', (), {'data': assign_data, 'query_params': request.query_params})())
    ...
    def validate_matrix(self, request):
        ...
        # Get gaps
        gaps = CompetencyGap.objects.filter(
            tenant=tenant,
            is_resolved=False
        )
        ...
        check.execution_time_seconds = (check.completed_at - check.started_at).total_seconds()
        check.save()
    ...
    def graph_analysis(self, request):
            ...
            for assignment in assignments[:5]:
                ...
                if trainer_quals.exists():
                    ...
                    paths.append({
                        ...
                        'path': [qual.qualification_code, assignment.unit.unit_code],
                        'length': 1,
        ...
        covered_units = assignments.filter(meets_requirements=True).values('unit').distinct().count()
        ...
        analysis = {
            ...
            'avg_compliance': assignments.aggregate(Avg('compliance_score'))['compliance_score__avg'] or 0
        }
    ...
    def generate_compliance_report(self, request):
        ...
        report_title = f'Trainer Matrix Compliance Report - {report_date.strftime("%Y-%m-%d")}'
        ...
        compliant = assignments.filter(meets_requirements=True).count()
        compliance_pct = (compliant / total_assignments * 100) if total_assignments > 0 else 0
        ...
        for trainer in trainers:
            ...
            trainer_compliant = trainer_assignments.filter(meets_requirements=True).count()
            trainer_reports.append({
                'trainer_id': trainer['trainer_id'],
                'trainer_name': trainer['trainer_name'],
                'total_units': trainer_assignments.count(),
                'compliant_units': trainer_compliant,
                'compliance_rate': (trainer_compliant / trainer_assignments.count() * 100) if trainer_assignments.count() > 0 else 0
            })
        ...
        for unit in units:
            unit_assignments = assignments.filter(unit__unit_code=unit['unit__unit_code'])
        ...
        if gap_summary['critical_gaps'] > 0:
            recommendations.append('Address critical gaps immediately - trainers cannot deliver assigned units')
        if compliance_pct < 70:
            recommendations.append('Overall compliance is below 70% - urgent review of trainer qualifications needed')
        if gap_summary['high_gaps'] > 5:
            recommendations.append('Multiple high-severity gaps identified - implement upskilling program')
...
"""
    ...
    def dashboard(self, request):
        ...
        verified_qualifications = qualifications.filter(verification_status='verified').count()
        expired_qualifications = qualifications.filter(verification_status='expired').count()
        ...
        overall_compliance = (compliant_assignments / total_assignments * 100) if total_assignments > 0 else 0
        ...
        # Recent checks
        recent_checks_qs = ComplianceCheck.objects.filter(tenant=tenant).order_by('-created_at')[:5]
        recent_checks = [
            {
        ...
        # Top gap types
        gap_types = gaps.values('gap_type').annotate(count=Count('id')).order_by('-count')[:5]
        top_gap_types = [
            {
                'gap_type': gt['gap_type'],
                'count': gt['count']
            }
            for gt in gap_types
        ]
        ...
        trainers_with_gaps = gaps.filter(is_resolved=False).values('trainer_id', 'trainer_name').annotate(
            gap_count=Count('id')
