...
class RulesetViewSet(viewsets.ModelViewSet):
    ...
    def activate(self, request, pk=None):
        ...
        try:
            ruleset.activate()
            return Response(
                {"status": "activated", "ruleset": RulesetSerializer(ruleset).data}
            )
...
class ReferenceTableViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.filter(valid_from__lte=now).filter(
...
class EligibilityRequestViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def evaluate(self, request, pk=None):
        ...
        if ruleset_id:
            ...
            ruleset = Ruleset.objects.filter(
                jurisdiction_code=eligibility_request.jurisdiction_code, status="active"
            ).first()
        ...
        return Response(
            EligibilityDecisionSerializer(decision).data, status=status.HTTP_200_OK
        )
    ...
    def check_eligibility(self, request, pk=None):
        ...
        approved_override = decision.overrides.filter(final_outcome="eligible").first()
...
class DecisionOverrideViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        ).filter(decision__request__tenant=self.request.user.tenant)
    ...
    def create(self, request):
        ...
        decision = get_object_or_404(
            EligibilityDecision, id=serializer.validated_data["decision_id"]
        )
        ...
        return Response(
            DecisionOverrideSerializer(override).data, status=status.HTTP_201_CREATED
        )
...
class EvidenceAttachmentViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        """Filter by request"""
        queryset = EvidenceAttachment.objects.select_related("request__tenant").filter(
    ...
    def verify(self, request, pk=None):
        ...
        return Response(EvidenceAttachmentSerializer(attachment).data)
