...
class Jurisdiction(models.Model):
    ...
    default_ruleset = models.ForeignKey(
        "Ruleset", on_delete=models.SET_NULL, null=True, blank=True, related_name="+"
    )
...
class Ruleset(models.Model):
    ...
    version = models.CharField(
        max_length=20, help_text="Semantic version (e.g., 1.7.2)"
    )
    ...
    checksum = models.CharField(
        max_length=64, editable=False, help_text="SHA256 of rule content"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_rulesets"
    )
    ...
    def activate(self):
        ...
        Ruleset.objects.filter(
            jurisdiction_code=self.jurisdiction_code, status="active"
...
class RulesetArtifact(models.Model):
    ...
    ruleset = models.ForeignKey(
        Ruleset, on_delete=models.CASCADE, related_name="artifacts"
    )
...
class ReferenceTable(models.Model):
    ...
    namespace = models.CharField(
        max_length=100, help_text="Data namespace (e.g., 'vic.concessions')"
    )
    ...
    source = models.CharField(
        max_length=200, help_text="Data source URL or description"
    )
...
class EligibilityRequest(models.Model):
    ...
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="eligibility_requests"
    )
    person_id = models.CharField(
        max_length=100, help_text="Person identifier in SMS/LMS"
    )
    ...
    evidence_refs = models.JSONField(
        default=list, help_text="References to attached evidence documents"
    )
    ...
    requested_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="eligibility_requests"
    )
...
class ExternalLookup(models.Model):
    ...
    request = models.ForeignKey(
        EligibilityRequest, on_delete=models.CASCADE, related_name="external_lookups"
    )

    # Provider
    provider = models.CharField(
        max_length=100, help_text="API provider (USI, visa, etc.)"
    )
...
class EligibilityDecision(models.Model):
    ...
    request = models.OneToOneField(
        EligibilityRequest, on_delete=models.CASCADE, related_name="decision"
    )
...
class DecisionOverride(models.Model):
    ...
    decision = models.ForeignKey(
        EligibilityDecision, on_delete=models.CASCADE, related_name="overrides"
    )
    ...
    final_outcome = models.CharField(
        max_length=20, choices=EligibilityDecision.OUTCOME_CHOICES
    )
    ...
    policy_version = models.CharField(
        max_length=20, help_text="Override policy version applied"
    )
...
class EvidenceAttachment(models.Model):
    ...
    request = models.ForeignKey(
        EligibilityRequest, on_delete=models.CASCADE, related_name="attachments"
    )
...
class WebhookEndpoint(models.Model):
    ...
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="webhook_endpoints"
    )
