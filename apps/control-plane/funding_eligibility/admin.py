...
class JurisdictionRequirementAdmin(admin.ModelAdmin):
    list_display = [
        'jurisdiction', 'name', 'code', 'funding_percentage',
    ...
    fieldsets = (
        ...
        ('Funding Details', {
            'fields': ('funding_percentage', 'student_contribution')
        }),
        ('API Integration', {
            'fields': ('api_endpoint', 'api_key_required')
        }),
        ('Additional Rules', {
            'fields': ('additional_rules',),
            'classes': ('collapse',)
        }),
        ('Validity Period', {
...
class EligibilityRuleAdmin(admin.ModelAdmin):
    list_display = [
        'name', 'rule_type', 'jurisdiction_requirement',
        'is_mandatory', 'priority', 'is_active'
    ]
...
class EligibilityCheckAdmin(admin.ModelAdmin):
    list_display = [
        'check_number', 'student_name', 'course_code',
        'jurisdiction', 'status', 'is_eligible',
        'eligibility_percentage', 'checked_at'
    ]
    list_filter = [
        'status', 'jurisdiction', 'is_eligible',
        'override_approved', 'prevents_enrollment'
    ]
    search_fields = [
        'check_number', 'student_first_name', 'student_last_name',
        'student_email', 'course_code', 'course_name'
    ]
    readonly_fields = [
        'check_number', 'checked_at', 'updated_at',
        'api_verified_at', 'override_approved_at'
    ]
    inlines = [EligibilityCheckLogInline]
    
    fieldsets = (
        ('Check Details', {
            'fields': ('tenant', 'check_number', 'status')
        }),
        ('Student Information', {
            'fields': (
                'student_first_name',
