...
def enqueue_external_lookups(self, request_id: int):
    ...
    try:
        ...
        for lookup_def in lookups_needed:
            perform_external_lookup.delay(request_id, lookup_def['provider'], lookup_def['data'])
...
def perform_external_lookup(self, request_id: int, provider: str, lookup_data: dict):
    ...
    try:
        ...
        logger.info(f"Completed {provider} lookup for request {request_id}: {result.success}")
        
    except Exception as e:
        logger.error(f"Error performing {provider} lookup for request {request_id}: {e}")
...
def evaluate_eligibility(self, request_id: int, ruleset_id: int = None):
    ...
    try:
        ...
        if pending_lookups > 0:
            logger.info(f"Request {request_id} has {pending_lookups} pending lookups, retrying...")
        ...
        if not ruleset:
            raise ValueError(f"No active ruleset for jurisdiction {request_obj.jurisdiction_code}")
...
def deliver_webhook(self, request_id: int, event_type: str):
    ...
    try:
        ...
        logger.info(f"Queued webhook delivery for request {request_id}, event {event_type}")
...
def deliver_webhook_to_endpoint(self, endpoint_id: int, payload: dict):
    ...
    try:
        ...
        if response.status_code >= 200 and response.status_code < 300:
            ...
            logger.warning(f"Webhook delivery failed to {endpoint.name}: {response.status_code}")
        ...
        logger.error(f"Unexpected error delivering webhook to endpoint {endpoint_id}: {e}")
        raise
...
def compute_eligibility_metrics():
    ...
    try:
        ...
        # Count by outcome
        outcomes = EligibilityDecision.objects.filter(
            decided_at__gte=since
        ).values('outcome').annotate(count=Count('id'))
        
        # Count by jurisdiction
        by_jurisdiction = EligibilityRequest.objects.filter(
            requested_at__gte=since
        ).values('jurisdiction_code').annotate(count=Count('id'))
