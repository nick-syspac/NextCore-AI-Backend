...
class JurisdictionRequirement(models.Model):
    ...
    code = models.CharField(
        max_length=50, help_text="Program code (e.g., 'SS-NSW', 'STS-VIC')"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_requirements"
    )
...
class EligibilityRule(models.Model):
    ...
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="eligibility_rules"
    )
    ...
    is_mandatory = models.BooleanField(
        default=True, help_text="Must pass for eligibility (vs. optional/bonus points)"
    )
    ...
    override_allowed = models.BooleanField(
        default=False, help_text="Can be manually overridden by authorized staff"
    )
...
class EligibilityCheck(models.Model):
    ...
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="eligibility_checks"
    )
    ...
    jurisdiction = models.CharField(
        max_length=20, choices=JurisdictionRequirement.JURISDICTION_CHOICES
    )
    jurisdiction_requirement = models.ForeignKey(
        JurisdictionRequirement,
    ...
    check_results = models.JSONField(
        default=dict, help_text="Detailed results of each rule evaluation"
    )
    ...
    api_response = models.JSONField(
        default=dict, blank=True, help_text="Response from external API verification"
    )
    ...
    valid_until = models.DateField(
        null=True, blank=True, help_text="Eligibility expiry date"
    )
    ...
    def __str__(self):
        return (
            f"{self.check_number} - {self.student_first_name} {self.student_last_name}"
        )
    ...
    def calculate_age(self):
        ...
        return (
            today.year
            - self.student_dob.year
            - (
                (today.month, today.day)
                < (self.student_dob.month, self.student_dob.day)
            )
        )
...
class EligibilityCheckLog(models.Model):
    ...
    eligibility_check = models.ForeignKey(
        EligibilityCheck, on_delete=models.CASCADE, related_name="logs"
    )
