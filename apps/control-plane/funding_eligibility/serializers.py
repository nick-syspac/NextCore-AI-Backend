...
class JurisdictionRequirementSerializer(serializers.ModelSerializer):
    jurisdiction_display = serializers.CharField(
        source="get_jurisdiction_display", read_only=True
    )
...
class EligibilityRuleSerializer(serializers.ModelSerializer):
    ...
    jurisdiction_name = serializers.CharField(
        source="jurisdiction_requirement.name", read_only=True
    )
...
class EligibilityCheckSerializer(serializers.ModelSerializer):
    ...
    jurisdiction_requirement_name = serializers.CharField(
        source="jurisdiction_requirement.name", read_only=True
    )
    student_age = serializers.SerializerMethodField()
    is_currently_valid = serializers.BooleanField(read_only=True)
    eligibility_summary = serializers.CharField(
        source="get_eligibility_summary", read_only=True
    )
    checked_by_name = serializers.CharField(
        source="checked_by.get_full_name", read_only=True
    )
    override_approved_by_name = serializers.CharField(
        source="override_approved_by.get_full_name", read_only=True
    )
...
class EligibilityCheckRequestSerializer(serializers.Serializer):
    ...
    jurisdiction = serializers.ChoiceField(
        choices=JurisdictionRequirement.JURISDICTION_CHOICES
    )
    funding_program_code = serializers.CharField(
    ...
    jurisdiction_residency_months = serializers.IntegerField(
    ...
    highest_aqf_level = serializers.IntegerField(
    ...
    concession_card_type = serializers.CharField(
