...
class RulesEngine:
    ...
    def evaluate(
        ...
        if passed:
            ...
            failed_rules = [r['artifact'] for r in rule_results if not r.get('passed', True)]
            
            # Heuristic: if certain rules fail, require manual review
            review_triggers = ['income_verification', 'qualification_assessment', 'special_case']
            needs_review = any(trigger in ' '.join(failed_rules) for trigger in review_triggers)
