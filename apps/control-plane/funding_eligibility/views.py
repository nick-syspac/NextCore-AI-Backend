...
class EligibilityCheckViewSet(viewsets.ModelViewSet):
    ...
    def check_eligibility(self, request):
        ...
        student_data = {
            ...
            'jurisdiction_residency_months': data.get('jurisdiction_residency_months', 0),
        ...
        # Run rules engine
        check_results = self._run_rules_engine(jurisdiction_req, data['aqf_level'], student_data)
        ...
        eligibility_check = EligibilityCheck.objects.create(
            ...
            funding_program_code=data.get('funding_program_code', jurisdiction_req.code),
            ...
            prevents_enrollment=not check_results['is_eligible'] and not check_results['override_required'],
        ...
        if jurisdiction_req.api_endpoint:
            api_result = self._call_verification_api(jurisdiction_req, eligibility_check, student_data)
            if api_result:
    ...
    def _run_rules_engine(self, jurisdiction_req, aqf_level, student_data):
        ...
        jr_checks = self._check_jurisdiction_requirements(jurisdiction_req, aqf_level, student_data)
    ...
    def _check_jurisdiction_requirements(self, req, aqf_level, student_data):
        ...
        if req.requires_australian_citizen:
            if citizenship == 'citizen':
                result['passed'] += 1
                result['details'].append({'check': 'citizenship', 'passed': True, 'message': 'Australian citizen'})
            else:
                result['failed'] += 1
                ...
                result['details'].append({'check': 'citizenship', 'passed': False, 'message': 'Not Australian citizen'})
        elif req.requires_permanent_resident:
            if citizenship in ['citizen', 'permanent_resident']:
                result['passed'] += 1
                result['details'].append({'check': 'citizenship', 'passed': True, 'message': f'{citizenship}'})
            else:
                result['failed'] += 1
                result['failed_mandatory'].append({
                ...
                result['details'].append({'check': 'citizenship', 'passed': False, 'message': f'{citizenship} not eligible'})
        else:
            result['passed'] += 1
            result['details'].append({'check': 'citizenship', 'passed': True, 'message': 'No citizenship restriction'})
        ...
        if req.requires_jurisdiction_resident:
            ...
            if student_data.get('is_jurisdiction_resident'):
                ...
                result['details'].append({'check': 'residency', 'passed': False, 'message': 'Not jurisdiction resident'})
        ...
        if req.min_age or req.max_age:
            ...
            if age_ok:
                result['passed'] += 1
                result['details'].append({'check': 'age', 'passed': True, 'message': age_msg})
            else:
                result['failed'] += 1
                result['failed_mandatory'].append({
                ...
                result['details'].append({'check': 'age', 'passed': False, 'message': age_msg})
        ...
        if req.requires_unemployed:
            ...
            if emp_status == 'unemployed':
                result['passed'] += 1
                result['details'].append({'check': 'employment', 'passed': True, 'message': 'Unemployed'})
            else:
                result['failed'] += 1
                result['failed_mandatory'].append({
                ...
                result['details'].append({'check': 'employment', 'passed': False, 'message': f'Status: {emp_status}'})
        elif req.requires_apprentice_trainee:
            result['checked'] += 1
            emp_status = student_data.get('employment_status')
            if emp_status in ['apprentice', 'trainee']:
                result['passed'] += 1
                result['details'].append({'check': 'employment', 'passed': True, 'message': emp_status.title()})
            else:
                result['failed'] += 1
                result['failed_mandatory'].append({
    ...
    def dashboard_stats(self, request):
        ...
        stats['by_jurisdiction'] = {item['jurisdiction']: item['count'] for item in by_jurisdiction}
