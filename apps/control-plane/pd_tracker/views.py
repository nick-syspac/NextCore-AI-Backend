...
from .serializers import (
    PDActivitySerializer,
    PDActivityDetailSerializer,
    TrainerProfileSerializer,
    TrainerProfileDetailSerializer,
    PDSuggestionSerializer,
    PDSuggestionDetailSerializer,
    ComplianceRuleSerializer,
    ComplianceCheckSerializer,
    ComplianceCheckDetailSerializer,
    LogActivityRequestSerializer,
    GenerateSuggestionsRequestSerializer,
    GenerateSuggestionsResponseSerializer,
    CheckCurrencyRequestSerializer,
    CheckCurrencyResponseSerializer,
...
class PDActivityViewSet(viewsets.ModelViewSet):
    ...
    def log_activity(self, request):
        ...
        activity = PDActivity.objects.create(
            ...
            maintains_vocational_currency=data.get(
        ...
        profile, created = TrainerProfile.objects.get_or_create(
            tenant=data["tenant"],
            trainer_id=data["trainer_id"],
            defaults={"trainer_name": data["trainer_name"]},
        )
    ...
    def check_currency(self, request):
        ...
        if profile.vocational_currency_status == "expired":
            ...
            recommendations.append(
                "Vocational currency expiring soon - plan PD activity"
            )
        ...
        if profile.industry_currency_status == "expired":
            ...
            recommendations.append(
                "Industry currency expiring soon - plan industry placement"
            )
        ...
        if profile.current_year_hours < profile.annual_pd_goal_hours:
            ...
            recommendations.append(
                f"Need {hours_needed:.1f} more hours to meet annual goal"
            )
...
class PDSuggestionViewSet(viewsets.ModelViewSet):
    ...
    def generate_suggestions(self, request):
        ...
        include_critical_only = serializer.validated_data.get(
        ...
        if profile.vocational_currency_status == "expired":
            ...
            compliance_risks.append(
                "Vocational currency expired - immediate action required"
            )

            suggestions.append(
                PDSuggestion.objects.create(
        ...
        if profile.industry_currency_status == "expired":
            ...
            compliance_risks.append(
                "Industry currency expired - immediate action required"
            )
...
class ComplianceCheckViewSet(viewsets.ModelViewSet):
    ...
    def dashboard(self, request):
        ...
        total_hours = (
            activities.filter(status="completed").aggregate(
        ...
        activities_last_30 = activities.filter(
            status="completed", start_date__gte=thirty_days_ago
        ).count()

        hours_last_30 = (
            activities.filter(
                status="completed", start_date__gte=thirty_days_ago
            ).aggregate(total=Sum("hours_completed"))["total"]
            or 0
        )
        ...
        trainers_current = (
            profiles.filter(
                Q(vocational_currency_status="current")
                | Q(industry_currency_status="current")
            )
            .distinct()
            .count()
        )
        ...
        trainers_expiring = (
            profiles.filter(
                Q(vocational_currency_status="expiring_soon")
                | Q(industry_currency_status="expiring_soon")
            )
            .distinct()
            .count()
        )
        ...
        trainers_expired = (
            profiles.filter(
                Q(vocational_currency_status="expired")
                | Q(industry_currency_status="expired")
            )
            .distinct()
            .count()
        )
        ...
        # Pending items
        pending_suggestions = PDSuggestion.objects.filter(
            status="pending_review"
        ).count()
        ...
        for i in range(6):
            ...
            monthly_hours.append(
                {"month": month_start.strftime("%b %Y"), "hours": float(month_total)}
            )
    ...
    def compliance_report(self, request):
        ...
        for profile in profiles:
            ...
            hours_completed = (
                activities.aggregate(total=Sum("hours_completed"))["total"] or 0
            )
            ...
            if issues:
                ...
                profile.vocational_currency_status == "expiring_soon"
                or profile.industry_currency_status == "expiring_soon"
            ):
        ...
        response_data = {
            ...
            "compliance_checks": ComplianceCheckDetailSerializer(
                compliance_checks, many=True
            ).data,
            "summary": {
                "compliance_rate": (
                    (compliant / profiles.count() * 100) if profiles.count() > 0 else 0
                ),
                "total_hours_period": sum(c.hours_completed for c in compliance_checks),
                "average_hours_per_trainer": (
                    sum(c.hours_completed for c in compliance_checks) / profiles.count()
                    if profiles.count() > 0
                    else 0
                ),
            },
