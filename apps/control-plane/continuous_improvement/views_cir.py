...
class CommentViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        tenant = self.get_tenant()
        queryset = (
            Comment.objects.filter(improvement_action__tenant=tenant)
            .select_related("improvement_action", "author", "parent")
...
class ClauseLinkViewSet(TenantScopedMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = ClauseLink.objects.filter(
            ...
            "improvement_action", "clause", "clause__standard", "created_by"
        )
    ...
    def by_clause(self, request, tenant_id=None, tenant_slug=None):
        ...
        if not clause_id:
            return Response(
                {"error": "clause_id required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class KPISnapshotViewSet(TenantScopedMixin, viewsets.ReadOnlyModelViewSet):
    ...
    def compute(self, request, tenant_id=None, tenant_slug=None):
        ...
        task = compute_kpi_snapshots.delay(
            tenant.id, period=period, period_start=period_start, period_end=period_end
        )

        return Response(
            {"message": "KPI computation started", "task_id": task.id},
...
class ImprovementActionCIRViewSet(TenantScopedMixin, viewsets.GenericViewSet):
    ...
    def ai_classify(self, request, tenant_id=None, tenant_slug=None, pk=None):
        ...
        task = classify_improvement_action.delay(
            action.id, auto_link_clauses=request.data.get("auto_link_clauses", True)
        )
    ...
    def compliance_dashboard(self, request, tenant_id=None, tenant_slug=None):
        ...
        closing_this_week = actions.filter(
            ...
            target_completion_date__lte=timezone.now().date()
        ...
        clause_data = (
            ClauseLink.objects.filter(improvement_action__tenant=tenant)
            .values("clause__clause_number", "clause__title", "clause__standard__name")
            .annotate(
        ...
        # SLA breaches
        sla_breaches = list(
            actions.filter(compliance_status="overdue")
            .values(
