...
class Comment(models.Model):
    ...
    author = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name='improvement_comments'
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class Attachment(models.Model):
    ...
    # Security & integrity
    sha256_hash = models.CharField(max_length=64, blank=True, help_text="File integrity hash")
...
class Verification(models.Model):
    ...
    evidence_reviewed = models.JSONField(
        default=list,
        help_text="List of evidence items reviewed"
    )
...
class ClauseLink(models.Model):
    ...
    rationale = models.TextField(
        blank=True,
        help_text="Explanation for the clause link"
    )
    ...
    created_by = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        null=True,
        related_name='created_clause_links'
    )
...
class SLAPolicy(models.Model):
    ...
    applies_to_priorities = models.JSONField(
        default=list,
        help_text="List of priorities this policy applies to"
    )
    applies_to_sources = models.JSONField(
        default=list,
        help_text="List of source types this policy applies to"
    )
    applies_to_categories = models.JSONField(
        default=list,
        help_text="List of category IDs this policy applies to"
    )
    
    # Escalation
    warning_days_before = models.IntegerField(
        default=7,
        help_text="Days before due date to trigger warning"
    )
    escalate_on_breach = models.BooleanField(
        default=True,
        help_text="Auto-escalate when SLA is breached"
    )
    escalation_recipients = models.JSONField(
        default=list,
        help_text="User IDs to notify on escalation"
    )
    ...
    def applies_to_action(self, action) -> bool:
        ...
        # Check priority
        if self.applies_to_priorities and action.priority not in self.applies_to_priorities:
            return False
...
class KPISnapshot(models.Model):
    ...
    metadata = models.JSONField(
        default=dict,
        help_text="Additional metric metadata and breakdown"
    )
...
class TaxonomyLabel(models.Model):
    ...
    tenant = models.ForeignKey(
        'tenants.Tenant',
        on_delete=models.CASCADE,
        related_name='taxonomy_labels'
    )
    
    # Label details
    key = models.CharField(max_length=100, help_text="Label key (e.g., 'type:non-conformance')")
    ...
    category = models.CharField(
        max_length=50,
        help_text="Label category (type, origin, risk, etc.)"
    )
...
class AIRun(models.Model):
    ...
    target_entity = models.CharField(
        max_length=50,
        help_text="Entity type (improvement_action, etc.)"
    )
    ...
    output_json = models.JSONField(
        default=dict,
        help_text="AI output/predictions"
    )
    ...
    latency_ms = models.IntegerField(null=True, blank=True, help_text="Response time in ms")
