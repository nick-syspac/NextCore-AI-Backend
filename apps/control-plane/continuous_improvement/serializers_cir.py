...
class EmbeddingSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if (
            request
            and request.parser_context.get("view")
            and hasattr(request.parser_context["view"], "action")
        ):
...
class AISummarizeRequestSerializer(serializers.Serializer):
    ...
    style = serializers.ChoiceField(
        choices=["concise", "detailed", "action-oriented"], default="action-oriented"
    )
