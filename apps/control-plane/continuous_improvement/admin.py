...
class ImprovementCategoryAdmin(admin.ModelAdmin):
    ...
    fieldsets = [
        ('Basic Information', {
            'fields': ['tenant', 'name', 'category_type', 'description', 'color_code']
        }),
        ('ASQA Alignment', {
            'fields': ['related_standards']
        }),
        ('Status', {
...
class ImprovementActionAdmin(admin.ModelAdmin):
    list_display = [
        'action_number', 'title', 'priority', 'status',
        'compliance_status', 'responsible_person', 'target_completion_date'
    ]
    list_filter = [
        'status', 'priority', 'compliance_status', 'source',
        'is_critical_compliance', 'created_at'
    ]
    search_fields = ['action_number', 'title', 'description', 'ai_summary']
    readonly_fields = [
        'created_at', 'updated_at', 'ai_processed_at',
        'compliance_status', 'progress_percentage', 'is_overdue', 'days_until_due'
    ]
    ...
    fieldsets = [
        ('Basic Information', {
            'fields': [
                'tenant', 'action_number', 'title', 'description',
        ...
        ('Status & Timeline', {
            'fields': [
                'status', 'compliance_status', 'progress_percentage',
                'identified_date', 'planned_start_date',
                'target_completion_date', 'actual_completion_date',
                'is_overdue', 'days_until_due'
            ]
        }),
        ('Assignment', {
            'fields': ['responsible_person', 'supporting_staff']
        }),
...
class ActionTrackingAdmin(admin.ModelAdmin):
    list_display = [
        'improvement_action', 'update_type', 'is_blocker',
        'progress_percentage', 'created_at', 'created_by'
    ]
    ...
    fieldsets = [
        ('Update Information', {
            'fields': [
                'improvement_action', 'update_type', 'update_text',
                'progress_percentage'
            ]
        }),
        ('Status Changes', {
...
class ImprovementReviewAdmin(admin.ModelAdmin):
    list_display = [
        'review_number', 'title', 'review_type', 'review_date',
        'total_actions_reviewed', 'reviewed_by'
    ]
    list_filter = ['review_type', 'review_date', 'created_at']
    search_fields = ['review_number', 'title', 'key_findings']
    readonly_fields = [
        'created_at', 'updated_at',
        'total_actions_reviewed', 'actions_completed',
        'actions_on_track', 'actions_at_risk', 'actions_overdue'
    ]
    ...
    fieldsets = [
        ('Basic Information', {
            'fields': [
                'tenant', 'review_number', 'title', 'review_type', 'review_date'
            ]
        }),
        ('Review Scope', {
            'fields': [
                'actions_reviewed', 'review_period_start', 'review_period_end'
            ]
        }),
        ('Statistics', {
            'fields': [
