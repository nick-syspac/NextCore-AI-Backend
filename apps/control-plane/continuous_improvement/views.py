...
from .serializers import (
    ImprovementCategorySerializer, ImprovementActionSerializer,
    ImprovementActionDetailSerializer, ActionTrackingSerializer,
    ImprovementReviewSerializer, AIClassificationRequestSerializer,
...
class ImprovementCategoryViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ImprovementCategory.objects.filter(
            tenant_id=tenant_id
...
class ImprovementActionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        tenant_id = self.kwargs.get('tenant_id')
        queryset = ImprovementAction.objects.filter(
            tenant_id=tenant_id
        ).select_related(
    ...
    def classify(self, request, tenant_id=None, pk=None):
        ...
        return Response({
            ...
            'action': ImprovementActionSerializer(action, context={'request': request}).data
        })
    ...
    def dashboard_stats(self, request, tenant_id=None):
        ...
        # By priority
        by_priority = dict(
            actions.values('priority').annotate(count=Count('id')).values_list('priority', 'count')
        )
        
        # By compliance status
        by_compliance = dict(
            actions.values('compliance_status').annotate(count=Count('id')).values_list('compliance_status', 'count')
        )
    ...
    def compliance_overview(self, request, tenant_id=None):
        ...
        for i in range(6):
            ...
            trends.append({
                ...
                'overdue': month_actions.filter(compliance_status='overdue').count(),
            })
        ...
        return Response({
            ...
            'critical': {
                ...
                'compliance_rate': round(
                    ((critical_total - critical_overdue) / critical_total * 100) if critical_total > 0 else 100,
                    1
                )
            },
    ...
    def _classify_text(self, description, title='', root_cause=''):
        ...
        category_keywords = {
            'training_assessment': ['training', 'assessment', 'tas', 'learning', 'teaching', 'delivery', 'curriculum'],
            'trainer_qualifications': ['trainer', 'assessor', 'qualification', 'competency', 'industry experience', 'tae'],
            'student_support': ['student', 'learner', 'support', 'wellbeing', 'accessibility', 'disability'],
            'facilities_equipment': ['facility', 'equipment', 'classroom', 'workshop', 'resources', 'tools'],
            'admin_records': ['records', 'documentation', 'administration', 'enrolment', 'usi', 'avetmiss'],
            'compliance_governance': ['compliance', 'governance', 'audit', 'regulation', 'asqa', 'policy'],
            'marketing_recruitment': ['marketing', 'recruitment', 'advertising', 'promotion', 'enrolment'],
            'financial_management': ['financial', 'budget', 'fees', 'refund', 'payment', 'invoice'],
            'quality_assurance': ['quality', 'validation', 'moderation', 'review', 'improvement'],
            'stakeholder_engagement': ['stakeholder', 'employer', 'industry', 'partnership', 'consultation'],
        }
        ...
        stop_words = {'with', 'from', 'that', 'this', 'have', 'been', 'will', 'their', 'about'}
        ...
        return {
            ...
            'category_display': dict(ImprovementAction._meta.get_field('status').choices).get(
                classified_category,
                classified_category.replace('_', ' ').title()
            )
        }
...
class ImprovementReviewViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        tenant_id = self.kwargs.get('tenant_id')
        return ImprovementReview.objects.filter(
            tenant_id=tenant_id
        ).prefetch_related('actions_reviewed', 'attendees')
    ...
    def generate_ai_insights(self, request, tenant_id=None, pk=None):
        ...
        # Category analysis
        category_counts = actions.values('category__name').annotate(count=Count('id')).order_by('-count')
        if category_counts:
