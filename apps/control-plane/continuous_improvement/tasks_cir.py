...
def classify_improvement_action(
    self, action_id: int, *, auto_link_clauses: bool = True
    ...
    try:
        action = ImprovementAction.objects.select_related("tenant", "category").get(
            pk=action_id
        )
    ...
    if any(
        word in text_lower
        for word in ["critical", "urgent", "severe", "registration", "compliance"]
    ):
...
def summarize_improvement_action(
    self, action_id: int, *, max_length: int = 80, style: str = "action-oriented"
) -> SummarizationResult:
    """
    ...
    logger.info(
        "Starting AI summarization", extra={"action_id": action_id, "style": style}
    )
...
Root Cause: {action.root_cause[:100] if action.root_cause else 'Not specified'}
    ...
    AIRun.objects.create(
        ...
        output_json={"summary": summary, "key_points": key_points, "style": style},
        success=True,
    ...
    return SummarizationResult(
        summary=summary, key_points=key_points, processing_time_ms=processing_time_ms
    )
...
def compute_kpi_snapshots(
    ...
    # Average time to close
    avg_days_to_close = (
        completed_actions.filter(actual_completion_date__isnull=False)
        .annotate(days_to_close=F("actual_completion_date") - F("identified_date"))
        .aggregate(avg=Avg("days_to_close"))["avg"]
    )

    avg_days_value = Decimal(str(avg_days_to_close.days) if avg_days_to_close else 0)

    # SLA compliance
    overdue_count = actions.filter(compliance_status="overdue").count()
    sla_compliance_rate = Decimal(
        ((total_actions - overdue_count) / total_actions * 100)
        if total_actions > 0
        else 100
    )
...
def _auto_link_clauses(
    action: ImprovementAction, standard_refs: List[str], text_lower: str
) -> int:
    """
