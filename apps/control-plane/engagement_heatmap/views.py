...
from .serializers import (
    EngagementHeatmapSerializer, AttendanceRecordSerializer,
    LMSActivitySerializer, DiscussionSentimentSerializer,
    EngagementAlertSerializer, HeatmapGenerationRequestSerializer
)
...
class EngagementHeatmapViewSet(viewsets.ModelViewSet):
    ...
    def generate_heatmap(self, request, tenant_slug=None):
        ...
        heatmap_data = self._generate_heatmap_data(
            student_id, tenant_slug, start_date, end_date,
            attendance_data, lms_data, sentiment_data
        )
        
        # Step 5: Identify risk flags
        risk_flags = self._identify_risk_flags(
            attendance_score, lms_score, sentiment_score,
            attendance_data, lms_data, sentiment_data
        )
    ...
    def _analyze_lms_activity(self, student_id, tenant_slug, start_date, end_date):
        ...
        frequency_score = min(100, (daily_average / 5) * 100)  # Target: 5 activities/day
        time_score = min(100, (daily_minutes / 60) * 100)  # Target: 60 min/day
        variety_score = min(100, (len(activity_breakdown) / 9) * 100)  # 9 activity types
    ...
    def _analyze_discussion_sentiment(self, student_id, tenant_slug, start_date, end_date):
    ...
    def _generate_heatmap_data(self, student_id, tenant_slug, start_date, end_date,
                               attendance_data, lms_data, sentiment_data):
        ...
        while current_date <= end_date:
            ...
            lms_minutes = LMSActivity.objects.filter(
                student_id=student_id,
                tenant=tenant_slug,
                date=current_date
    ...
    def _identify_risk_flags(self, attendance_score, lms_score, sentiment_score,
                            attendance_data, lms_data, sentiment_data):
        ...
        if sentiment_data.get('negative_count', 0) > sentiment_data.get('positive_count', 0):
            flags.append('sentiment_decline')
        
        # Combined risks
        overall_score = (attendance_score * 0.4) + (lms_score * 0.35) + (sentiment_score * 0.25)
        if overall_score < 40:
    ...
    def risk_dashboard(self, request, tenant_slug=None):
        ...
        risk_breakdown = heatmaps.values('risk_level').annotate(
            count=Count('id')
        )
        
        # Active alerts
        active_alerts = EngagementAlert.objects.filter(
            tenant=tenant_slug,
            status='active'
        ...
        # Trend analysis
        trends = heatmaps.values('engagement_trend').annotate(
            count=Count('id')
        )
