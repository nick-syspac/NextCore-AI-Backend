...
class ASQAClauseSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ASQAClause
        fields = [
            'id', 'standard', 'clause_number', 'title', 'clause_text',
            'evidence_required', 'keywords', 'compliance_level',
            'compliance_level_display', 'is_active', 'created_at', 'updated_at'
        ]
...
class ASQAStandardSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = ASQAStandard
        fields = [
            'id', 'standard_number', 'title', 'description', 'standard_type',
            'standard_type_display', 'full_text', 'requirements', 'is_active',
...
class PolicySerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Policy
        fields = [
            'id', 'tenant', 'policy_number', 'title', 'description',
            'policy_type', 'policy_type_display', 'content', 'version',
            'status', 'status_display', 'effective_date', 'review_date',
            'last_compared_at', 'compliance_score', 'file_path',
            'created_by', 'created_by_details', 'created_at', 'updated_at',
            'comparison_count', 'compliance_status'
        ]
        read_only_fields = ['last_compared_at', 'compliance_score', 'created_at', 'updated_at']
...
class ComparisonResultSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'policy', 'policy_details', 'asqa_clause', 'asqa_clause_details',
            'similarity_score', 'match_type', 'match_type_display',
            'matched_text', 'gap_description', 'recommendations',
            'nlp_metadata', 'keywords_matched', 'keywords_missing',
            'has_sufficient_evidence', 'evidence_notes', 'is_compliant',
            'requires_action', 'comparison_date', 'reviewed_by',
            'reviewed_by_details', 'reviewed_at'
        ]
...
class ComparisonSessionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'tenant', 'policy', 'policy_details', 'session_name',
            'status', 'status_display', 'standards_compared',
            'total_clauses_checked', 'compliant_count', 'partial_match_count',
            'gap_count', 'overall_compliance_score', 'processing_time_seconds',
            'error_message', 'created_by', 'created_by_details',
...
class CompareRequestSerializer(serializers.Serializer):
    ...
    use_nlp = serializers.BooleanField(default=True, help_text="Use NLP-based text similarity")
