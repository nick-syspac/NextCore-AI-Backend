...
from .serializers import (
    ...
    VerificationScanSerializer,
    VerificationScanListSerializer,
    LinkedInActivitySerializer,
    GitHubActivitySerializer,
    CurrencyEvidenceSerializer,
    EntityExtractionSerializer,
    StartScanRequestSerializer,
    StartScanResponseSerializer,
    ScanLinkedInRequestSerializer,
    ScanLinkedInResponseSerializer,
    ScanGitHubRequestSerializer,
    ScanGitHubResponseSerializer,
    AnalyzeCurrencyRequestSerializer,
    AnalyzeCurrencyResponseSerializer,
    GenerateEvidenceRequestSerializer,
    GenerateEvidenceResponseSerializer,
    ExtractEntitiesRequestSerializer,
    ExtractEntitiesResponseSerializer,
    DashboardStatsSerializer,
    VerifyProfileRequestSerializer,
    VerifyProfileResponseSerializer,
)
...
class TrainerProfileViewSet(viewsets.ModelViewSet):
    ...
    def start_scan(self, request):
        ...
        scan = VerificationScan.objects.create(
            ...
            sources_to_scan=serializer.validated_data.get(
    ...
    def scan_linkedin(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Verification scan not found"},
        ...
        for activity_data in sample_activities[
            : serializer.validated_data.get("max_items", 50)
        ]:
    ...
    def analyze_currency(self, request):
        ...
        recent_linkedin = linkedin_activities.filter(
            activity_date__gte=recent_threshold
        ).count()
        recent_github = github_activities.filter(
            activity_date__gte=recent_threshold
        ).count()
        total_activities = linkedin_activities.count() + github_activities.count()
        recency_score = (
            ((recent_linkedin + recent_github) / max(total_activities, 1)) * 100
        ...
        avg_linkedin_relevance = (
            relevant_linkedin.aggregate(Avg("relevance_score"))["relevance_score__avg"]
            or 0
        )
        avg_github_relevance = (
            relevant_github.aggregate(Avg("relevance_score"))["relevance_score__avg"]
            or 0
        )
        ...
        if recency_score < 70:
            recommendations.append(
                "Increase recent professional activities and contributions"
            )
        if relevance_score < 70:
            recommendations.append(
                "Focus on activities more directly related to your industry"
            )
        if frequency_score < 70:
            recommendations.append(
                "Maintain more consistent engagement with professional platforms"
            )
        if not recommendations:
            recommendations.append(
                "Excellent currency! Continue maintaining current activity levels"
            )
        ...
        profile.next_verification_date = timezone.now().date() + timedelta(
            days=profile.verification_frequency_days
        )
        profile.save()
...
"""
    ...
    def extract_entities(self, request):
        ...
        extraction = EntityExtraction.objects.create(
            ...
            nlp_model_used=serializer.validated_data.get(
                "nlp_model", "spacy-en_core_web_lg"
            ),
    ...
    def dashboard(self, request):
        ...
        total_scans = VerificationScan.objects.filter(
            trainer_profile__tenant=tenant
        ).count()
        scans_this_month = VerificationScan.objects.filter(
            trainer_profile__tenant=tenant, created_at__gte=month_ago
        ).count()

        avg_currency_score = (
            all_profiles.aggregate(Avg("currency_score"))["currency_score__avg"] or 0
        )
        ...
        # Top industries
        industries = (
            all_profiles.values("primary_industry")
            .annotate(count=Count("id"))
            .order_by("-count")[:5]
        )
        top_industries = [
            {"industry": i["primary_industry"], "count": i["count"]} for i in industries
        ]
    ...
    def verify_profile(self, request):
        ...
        if (
            serializer.validated_data.get("scan_linkedin", True)
        ...
        if serializer.validated_data.get("generate_evidence", True):
            evidence = CurrencyEvidence.objects.create(
                trainer_profile=profile,
                verification_scan=scan,
                evidence_type=serializer.validated_data.get(
    ...
    def get_queryset(self):
        ...
        if is_relevant is not None:
            queryset = queryset.filter(
                is_industry_relevant=is_relevant.lower() == "true"
            )
