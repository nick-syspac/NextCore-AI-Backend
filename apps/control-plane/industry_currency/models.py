...
class VerificationScan(models.Model):
    ...
    trainer_profile = models.ForeignKey(TrainerProfile, on_delete=models.CASCADE, related_name='scans')
    ...
    sources_to_scan = models.JSONField(default=list)  # ['linkedin', 'github', 'twitter']
...
class LinkedInActivity(models.Model):
    ...
    verification_scan = models.ForeignKey(VerificationScan, on_delete=models.CASCADE, related_name='linkedin_activities')
...
class GitHubActivity(models.Model):
    ...
    activity_number = models.CharField(max_length=50, unique=True, db_index=True)
    verification_scan = models.ForeignKey(VerificationScan, on_delete=models.CASCADE, related_name='github_activities')
...
class CurrencyEvidence(models.Model):
    ...
    trainer_profile = models.ForeignKey(TrainerProfile, on_delete=models.CASCADE, related_name='currency_evidence')
    verification_scan = models.ForeignKey(VerificationScan, on_delete=models.CASCADE, related_name='evidence_documents')
    ...
    # Evidence metadata
    linkedin_activities_included = models.JSONField(default=list)  # List of activity IDs
...
class EntityExtraction(models.Model):
    ...
    verification_scan = models.ForeignKey(VerificationScan, on_delete=models.CASCADE, related_name='entity_extractions')
    ...
    # Processing metadata
    nlp_model_used = models.CharField(max_length=100, blank=True)  # e.g., "spacy-en_core_web_lg"
