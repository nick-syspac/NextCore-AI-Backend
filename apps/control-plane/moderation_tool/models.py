...
class ModerationSession(models.Model):
    ...
    assessment_type = models.CharField(
        max_length=50, choices=ASSESSMENT_TYPE_CHOICES, default="exam"
    )
    ...
    average_agreement_rate = models.FloatField(
    ...
    def get_fairness_score(self):
        ...
        outlier_penalty = min(
            (self.outliers_detected / max(self.decisions_compared, 1)) * 25, 25
        )
        bias_penalty = min(
            (self.bias_flags_raised / max(self.decisions_compared, 1)) * 25, 25
        )
...
class AssessorDecision(models.Model):
    ...
    session = models.ForeignKey(
        ModerationSession, on_delete=models.CASCADE, related_name="decisions"
    )
...
class OutlierDetection(models.Model):
    ...
    outlier_number = models.CharField(
        max_length=50, unique=True, blank=True, editable=False
    )
    session = models.ForeignKey(
        ModerationSession, on_delete=models.CASCADE, related_name="outliers"
    )
    decision = models.ForeignKey(
        AssessorDecision, on_delete=models.CASCADE, related_name="outlier_flags"
    )
    ...
    deviation_percentage = models.FloatField(
        help_text="Percentage deviation from average"
    )
    ...
    assessor_mean = models.FloatField(
...
class BiasScore(models.Model):
    ...
    bias_number = models.CharField(
        max_length=50, unique=True, blank=True, editable=False
    )
    session = models.ForeignKey(
        ModerationSession, on_delete=models.CASCADE, related_name="bias_scores"
    )
    ...
    evidence = models.JSONField(
        default=dict, help_text="Statistical evidence and patterns detected"
    )
...
class ModerationLog(models.Model):
    ...
    session = models.ForeignKey(
        ModerationSession, on_delete=models.CASCADE, related_name="logs"
    )
