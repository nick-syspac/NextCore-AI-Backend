...
from .serializers import (
    ModerationSessionSerializer, AssessorDecisionSerializer,
    OutlierDetectionSerializer, BiasScoreSerializer, ModerationLogSerializer,
    CompareDecisionsRequestSerializer, DetectOutliersRequestSerializer,
...
class ModerationSessionViewSet(viewsets.ModelViewSet):
    ...
    def detect_outliers(self, request, pk=None):
        ...
        threshold = serializer.validated_data.get('threshold', session.outlier_threshold)
        ...
        cohort_std_dev = statistics.stdev(percentage_scores) if len(percentage_scores) > 1 else 0
        
        # Calculate assessor averages
        assessor_stats = {}
        for decision in decisions:
            if decision.assessor_id not in assessor_stats:
                assessor_decisions = [d for d in decisions if d.assessor_id == decision.assessor_id]
        ...
        for decision in decisions:
            ...
            if abs(z_score) >= threshold:
                ...
                deviation_percentage = ((percentage_score - cohort_mean) / cohort_mean) * 100 if cohort_mean > 0 else 0
        ...
        return Response({
            ...
            'total_decisions': len(decisions),
            'detection_rate': len(outliers_detected) / len(decisions) if decisions else 0,
            'cohort_statistics': {
                'mean': round(cohort_mean, 2),
                'std_dev': round(cohort_std_dev, 2),
            },
            'outliers': OutlierDetectionSerializer(outliers_detected, many=True).data
        })
    ...
    def calculate_bias(self, request, pk=None):
        ...
        bias_types = serializer.validated_data.get('bias_types', ['leniency', 'severity', 'central_tendency'])
        ...
        cohort_std_dev = statistics.stdev(cohort_scores) if len(cohort_scores) > 1 else 0
        
        bias_scores = []
        
        for assessor_id, assessor_name in assessors_to_check:
            assessor_decisions = [d for d in all_decisions if d.assessor_id == assessor_id]
            ...
            assessor_std_dev = statistics.stdev(assessor_scores) if len(assessor_scores) > 1 else 0
            
            mean_difference = assessor_mean - cohort_mean
            std_dev_ratio = assessor_std_dev / cohort_std_dev if cohort_std_dev > 0 else 1.0
