...
class LearningPathway(models.Model):
    ...
    estimated_duration_hours = models.DecimalField(max_digits=6, decimal_places=1, validators=[MinValueValidator(0)])
...
class LearningStep(models.Model):
    ...
    pathway = models.ForeignKey(LearningPathway, on_delete=models.CASCADE, related_name='steps')
...
class StudentProgress(models.Model):
    ...
    tenant = models.CharField(max_length=100, db_index=True)
    student_id = models.CharField(max_length=100, db_index=True)
    pathway = models.ForeignKey(LearningPathway, on_delete=models.CASCADE, related_name='progress_records')
    step = models.ForeignKey(LearningStep, on_delete=models.CASCADE, related_name='progress_records')
...
class PathwayRecommendation(models.Model):
    ...
    algorithm_used = models.CharField(max_length=30, choices=ALGORITHM_CHOICES, default='hybrid')
    ...
    recommendation_reasons = models.JSONField(
        default=list,
        help_text="Human-readable reasons for recommendation"
    )
...
class ContentEmbedding(models.Model):
    ...
    step = models.OneToOneField(LearningStep, on_delete=models.CASCADE, related_name='embedding')
