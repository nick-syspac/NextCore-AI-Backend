...
class AutoMarker(models.Model):
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_markers"
    )

    # Configuration
    answer_type = models.CharField(
        max_length=50, choices=ANSWER_TYPE_CHOICES, default="short_answer"
    )
    ...
    similarity_model = models.CharField(
        max_length=50, choices=SIMILARITY_MODEL_CHOICES, default="sentence_transformers"
    )
    ...
    # Marking criteria
    use_keywords = models.BooleanField(
        default=True, help_text="Enable keyword matching"
    )
    keywords = models.JSONField(
        default=list, blank=True, help_text="Required keywords for full marks"
    )
    ...
    average_marking_time = models.FloatField(
    ...
    def get_marking_statistics(self):
        ...
        return {
            ...
            "pass_rate": responses.filter(
                marks_awarded__gte=self.max_marks * 0.5
            ).count()
            / total
            * 100,
        }
...
class MarkedResponse(models.Model):
    ...
    auto_marker = models.ForeignKey(
        AutoMarker, on_delete=models.CASCADE, related_name="responses"
    )
    ...
    similarity_breakdown = models.JSONField(
        default=dict, blank=True, help_text="Detailed similarity analysis"
    )
    ...
    marking_time = models.FloatField(
...
class MarkingCriterion(models.Model):
    ...
    auto_marker = models.ForeignKey(
        AutoMarker, on_delete=models.CASCADE, related_name="criteria"
    )
...
class CriterionScore(models.Model):
    ...
    response = models.ForeignKey(
        MarkedResponse, on_delete=models.CASCADE, related_name="criterion_scores"
    )
...
class MarkingLog(models.Model):
    ...
    # Reference
    auto_marker = models.ForeignKey(
        AutoMarker, on_delete=models.CASCADE, related_name="logs"
    )
    response = models.ForeignKey(
        MarkedResponse,
