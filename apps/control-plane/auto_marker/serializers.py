...
class MarkedResponseListSerializer(serializers.ModelSerializer):
    ...
    auto_marker_title = serializers.CharField(
        source="auto_marker.title", read_only=True
    )
...
class MarkResponsesRequestSerializer(serializers.Serializer):
    ...
    responses = serializers.ListField(
        child=serializers.DictField(), help_text="List of responses to mark"
    )
        ...
        for idx, response in enumerate(value):
            if "student_id" not in response:
                raise serializers.ValidationError(
                    f"Response {idx}: student_id is required"
                )
            if "student_name" not in response:
                raise serializers.ValidationError(
                    f"Response {idx}: student_name is required"
                )
            if "response_text" not in response:
                raise serializers.ValidationError(
                    f"Response {idx}: response_text is required"
                )
