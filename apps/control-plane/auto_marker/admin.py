...
class AutoMarkerAdmin(admin.ModelAdmin):
    list_display = [
        'marker_number', 'title', 'tenant', 'answer_type', 'status',
        'total_responses_marked', 'average_similarity_score', 'created_at'
    ]
    ...
    readonly_fields = [
        'marker_number', 'total_responses_marked', 'average_similarity_score',
        'average_marking_time', 'created_at', 'updated_at'
    ]
    
    fieldsets = (
        ('Identification', {
            'fields': ('marker_number', 'title', 'description', 'tenant', 'created_by')
        }),
        ...
        ('Semantic Similarity Settings', {
            'fields': (
                ...
                'partial_credit_enabled', 'min_similarity_for_credit'
            )
        }),
        ('Keyword Matching', {
        ...
        ('Performance Metrics', {
            'fields': (
                'total_responses_marked', 'average_similarity_score', 'average_marking_time'
            )
        }),
        ('Status', {
...
class MarkedResponseAdmin(admin.ModelAdmin):
    list_display = [
        'response_number', 'student_name', 'student_id', 'auto_marker',
        'marks_awarded', 'similarity_score', 'confidence_score',
        'requires_review', 'status', 'marked_at'
    ]
    list_filter = [
        'status', 'requires_review', 'auto_marker', 'marked_at', 'created_at'
    ]
    search_fields = [
        'response_number', 'student_id', 'student_name', 'response_text'
    ]
    readonly_fields = [
        'response_number', 'word_count', 'similarity_score',
        'keyword_match_score', 'combined_score', 'marks_awarded',
        'confidence_score', 'matched_keywords', 'missing_keywords',
        'key_phrases_detected', 'similarity_breakdown', 'automated_feedback',
        'marking_time', 'marked_at', 'created_at', 'updated_at'
    ]
    
    fieldsets = (
        ('Identification', {
            'fields': ('response_number', 'auto_marker', 'status')
        }),
        ('Student Information', {
            'fields': ('student_id', 'student_name')
        }),
        ('Response Content', {
            'fields': ('response_text', 'word_count')
        }),
        ('Scoring', {
            'fields': (
                'similarity_score', 'keyword_match_score', 'combined_score',
                'marks_awarded', 'confidence_score'
            )
        }),
        ('Analysis', {
            'fields': (
        ...
        ('Feedback & Review', {
            'fields': (
                'automated_feedback', 'requires_review', 'review_reason',
...
class MarkingCriterionAdmin(admin.ModelAdmin):
    list_display = [
        'criterion_name', 'auto_marker', 'weight', 'max_points',
    ...
    fieldsets = (
        ...
        ('Expected Content', {
            'fields': ('expected_content', 'criterion_keywords')
        }),
        ('Scoring', {
            'fields': ('weight', 'max_points', 'required')
        }),
        ('Display', {
...
class CriterionScoreAdmin(admin.ModelAdmin):
    list_display = [
        'response', 'criterion', 'similarity_score', 'points_awarded'
    ]
...
class MarkingLogAdmin(admin.ModelAdmin):
    list_display = [
        'auto_marker', 'action', 'performed_by', 'responses_processed',
        'average_time_per_response', 'timestamp'
    ]
    ...
    fieldsets = (
        ...
        ('AI/Model Information', {
            'fields': ('similarity_model', 'model_version')
        }),
        ('Performance Metrics', {
            'fields': (
                'responses_processed', 'total_time', 'average_time_per_response'
            )
        }),
        ('Score Changes', {
