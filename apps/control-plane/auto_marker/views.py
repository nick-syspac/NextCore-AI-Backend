...
from .serializers import (
    AutoMarkerListSerializer, AutoMarkerDetailSerializer,
    MarkedResponseListSerializer, MarkedResponseDetailSerializer,
    MarkingCriterionSerializer, CriterionScoreSerializer,
    MarkingLogSerializer, MarkResponsesRequestSerializer,
...
class AutoMarkerViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related('created_by').prefetch_related('criteria', 'responses')
    ...
    def mark_responses(self, request, tenant_slug=None, pk=None):
        ...
        enable_review_flagging = serializer.validated_data.get('enable_review_flagging', True)
        ...
        if auto_marker.total_responses_marked > 0:
            total_marking_time = (auto_marker.average_marking_time * (auto_marker.total_responses_marked - len(marked_responses))) + total_time
            auto_marker.average_marking_time = total_marking_time / auto_marker.total_responses_marked
        auto_marker.save()
        ...
        return Response({
            ...
            'total_time': total_time,
            'average_time': total_time / len(marked_responses) if marked_responses else 0,
    ...
    def _mark_response(self, response, auto_marker, enable_review_flagging=True):
        ...
        response.key_phrases_detected = self._extract_key_phrases(response.response_text)
        
        # Similarity breakdown
        response.similarity_breakdown = {
            'lexical_similarity': similarity_score * 0.4,
            'semantic_similarity': similarity_score * 0.6,
            'structure_match': 0.75 if len(response.response_text.split()) > 20 else 0.5,
            'completeness': min(1.0, len(response.response_text.split()) / len(auto_marker.model_answer.split()))
        }
        ...
        if enable_review_flagging:
            if response.confidence_score < 0.70:
                ...
                response.review_reason = f"Low confidence: {response.confidence_score:.2f}"
            elif len(response.missing_keywords) > len(auto_marker.keywords) / 2:
                response.requires_review = True
                response.review_reason = f"Missing {len(response.missing_keywords)} key concepts"
    ...
    def _generate_feedback(self, response, auto_marker):
        ...
        if percentage >= 90:
            feedback_parts.append("Excellent response! Your answer demonstrates strong understanding.")
        elif percentage >= 70:
            feedback_parts.append("Good response with solid understanding of the key concepts.")
        elif percentage >= 50:
            feedback_parts.append("Satisfactory response. Consider expanding on key points.")
        else:
            feedback_parts.append("Your response needs improvement. Review the model answer.")
        
        # Keyword feedback
        if response.missing_keywords:
            feedback_parts.append(f"Missing key concepts: {', '.join(response.missing_keywords[:3])}")
        
        # Similarity feedback
        if response.similarity_score < 0.6:
            feedback_parts.append("Try to align your response more closely with the expected answer structure.")
    ...
    def statistics(self, request, tenant_slug=None, pk=None):
        ...
        stats['recent_responses'] = MarkedResponseListSerializer(
            responses.order_by('-marked_at')[:10],
            many=True
        ).data
...
class MarkedResponseViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return queryset.select_related('auto_marker', 'reviewed_by').prefetch_related('criterion_scores')
