...
from .serializers import (
    ...
    UploadAudioRequestSerializer,
    UploadAudioResponseSerializer,
    ...
    CreateDailySummaryRequestSerializer,
    CreateDailySummaryResponseSerializer,
    GenerateEvidenceRequestSerializer,
    GenerateEvidenceResponseSerializer,
    DashboardStatsSerializer,
    ExportEvidenceRequestSerializer,
    ExportEvidenceResponseSerializer,
)
...
class DiaryEntryViewSet(viewsets.ModelViewSet):
    ...
    def upload_audio(self, request):
        ...
        try:
            diary_entry = DiaryEntry.objects.get(
                id=serializer.validated_data["diary_entry_id"]
            )
        except DiaryEntry.DoesNotExist:
            return Response(
                {"error": "Diary entry not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def transcribe_audio(self, request):
        ...
        try:
            ...
            return Response(
                {"error": "Audio recording not found"}, status=status.HTTP_404_NOT_FOUND
            )

        # Create transcription job
        job = TranscriptionJob.objects.create(
            audio_recording=recording,
            transcription_engine=serializer.validated_data.get(
                "transcription_engine", "whisper"
            ),
            language=serializer.validated_data.get("language", "en"),
            enable_speaker_diarization=serializer.validated_data.get(
    ...
    def generate_summary(self, request):
        ...
        content_parts = []
        if (
            serializer.validated_data.get("include_transcript", True)
            and diary_entry.raw_transcript
        ):
            content_parts.append(f"Transcript: {diary_entry.raw_transcript}")
        if (
            serializer.validated_data.get("include_manual_notes", True)
            and diary_entry.manual_notes
        ):
        ...
        summary_text = (
            f"Summary of teaching session for {diary_entry.course_name} on {diary_entry.session_date}. "
    ...
    def create_daily_summary(self, request):
        ...
        entries_query = DiaryEntry.objects.filter(
            trainer_id=trainer_id, session_date=summary_date
        )
        ...
        response_data = {
            ...
            "total_teaching_hours": total_hours,
            "message": (
                "Daily summary created successfully"
                if created
                else "Daily summary updated successfully"
            ),
        }
...
"""
    ...
    def dashboard(self, request):
        ...
        total_teaching_hours = (
            all_entries.aggregate(total=Sum("session_duration_minutes"))["total"] or 0
        )
        total_teaching_hours = total_teaching_hours / 60.0

        total_students_taught = (
            all_entries.aggregate(total=Sum("student_count"))["total"] or 0
        )

        total_recordings = AudioRecording.objects.filter(
            diary_entry__tenant=tenant, diary_entry__trainer_id=trainer_id
        ).count()
        ...
        daily_summaries_count = DailySummary.objects.filter(
            tenant=tenant, trainer_id=trainer_id
        ).count()
        ...
        # Courses taught
        courses_taught = list(
            all_entries.values_list("course_name", flat=True).distinct()
        )
    ...
    def export_evidence(self, request):
        ...
        export_file_path = (
            f"/exports/evidence_{trainer_id}_{start_date}_{end_date}.{export_format}"
        )
