...
class Evidence(models.Model):
    ...
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="evidence_documents"
    )
    ...
    uploaded_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="uploaded_evidence"
    )
...
class ClauseEvidence(models.Model):
    ...
    asqa_clause = models.ForeignKey(
        "policy_comparator.ASQAClause", on_delete=models.CASCADE
    )
    ...
    confidence_score = models.FloatField(
        default=0.0, help_text="Confidence score (0.0-1.0) for auto-tagged matches"
    )
    ...
    matched_keywords = models.JSONField(
        default=list, help_text="Keywords that matched between clause and evidence"
    )

    # Rule mapping metadata
    rule_name = models.CharField(
        max_length=100, blank=True, help_text="Name of rule that triggered mapping"
    )
...
class AuditReport(models.Model):
    ...
    # Basic info
    tenant = models.ForeignKey(
        "tenants.Tenant", on_delete=models.CASCADE, related_name="audit_reports"
    )
    ...
    auto_tagged_count = models.IntegerField(
    ...
    recommendations = models.JSONField(
        default=list, help_text="Overall recommendations"
    )

    # Timestamps
    created_at = models.DateTimeField(auto_now_add=True)
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_audit_reports"
    )
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def calculate_metrics(self):
        ...
        all_clauses = ASQAClause.objects.filter(standard__in=self.asqa_standards.all())
        ...
        if self.total_clauses > 0:
            self.compliance_percentage = round(
                (clauses_with_evidence / self.total_clauses) * 100, 2
            )
        else:
        ...
        critical_with_evidence = (
            critical_clauses.filter(
                clauseevidence__evidence__tenant=self.tenant,
                clauseevidence__evidence__status__in=["tagged", "reviewed", "approved"],
            )
            .distinct()
        ...
        self.auto_tagged_count = (
            ClauseEvidence.objects.filter(
                evidence__in=evidence_docs, mapping_type__in=["auto_ner", "auto_rule"]
            )
            .values("evidence")
            .distinct()
            .count()
        ...
        self.manually_tagged_count = (
            ClauseEvidence.objects.filter(
                evidence__in=evidence_docs, mapping_type="manual"
            )
            .values("evidence")
            .distinct()
            .count()
        ...
        self.verified_evidence_count = (
            ClauseEvidence.objects.filter(evidence__in=evidence_docs, is_verified=True)
            .values("evidence")
            .distinct()
            .count()
        )
...
class AuditReportClause(models.Model):
    ...
    audit_report = models.ForeignKey(
        AuditReport, on_delete=models.CASCADE, related_name="clause_entries"
    )
    asqa_clause = models.ForeignKey(
        "policy_comparator.ASQAClause", on_delete=models.CASCADE
    )
    ...
    compliance_status = models.CharField(
        max_length=20, choices=COMPLIANCE_STATUS_CHOICES, default="not_assessed"
    )
    ...
    # Evidence references
    linked_evidence = models.ManyToManyField(
        Evidence, related_name="audit_clause_entries", blank=True
    )
