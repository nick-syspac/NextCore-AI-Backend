...
from .serializers import (
    EvidenceSerializer,
    ClauseEvidenceSerializer,
    AuditReportSerializer,
    AuditReportDetailSerializer,
    AuditReportClauseSerializer,
    EvidenceUploadSerializer,
    ClauseEvidenceGapSerializer,
)
...
class TenantScopedViewSetMixin:
    ...
    def get_tenant(self) -> Tenant:
        ...
        tenant_id = self.kwargs.get("tenant_id") or getattr(
...
class EvidenceViewSet(TenantScopedViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            Evidence.objects.filter(tenant=tenant)
    ...
    def tagged_clauses(self, request, _tenant_id=None, pk=None):
        ...
        evidence = self.get_object()
        mappings = (
            ClauseEvidence.objects.filter(evidence=evidence)
            .select_related("asqa_clause", "asqa_clause__standard", "verified_by")
            .order_by("-confidence_score")
        )

        serializer = ClauseEvidenceSerializer(
    ...
    def verify_tagging(self, request, _tenant_id=None, pk=None):
        ...
        updated_count = ClauseEvidence.objects.filter(
            id__in=clause_evidence_ids, evidence=evidence
        ).update(is_verified=True, verified_by=request.user, verified_at=timezone.now())
...
class ClauseEvidenceViewSet(TenantScopedViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        tenant = self.get_tenant()
        return ClauseEvidence.objects.filter(evidence__tenant=tenant).select_related(
            "asqa_clause", "asqa_clause__standard", "evidence", "verified_by"
        )
        ...
        for clause in clauses_query.select_related("standard"):
            ...
            if clause.compliance_level == "critical":
                gap_severity = (
                    "critical"
                    if evidence_count == 0
                    else ("major" if verified_count < 2 else "none")
                )
            elif clause.compliance_level == "essential":
                gap_severity = (
                    "major"
                    if evidence_count == 0
                    else ("minor" if verified_count < 1 else "none")
                )
            else:
            ...
            if evidence_count == 0:
                ...
                recommendations.append(
                    f"Review policies related to: {', '.join(clause.keywords[:3]) if clause.keywords else 'this clause'}"
                )
            elif verified_count < 2:
                recommendations.append(
                    f"Verify existing evidence for {clause.clause_number}"
                )
...
class AuditReportViewSet(TenantScopedViewSetMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return AuditReport.objects.filter(tenant=tenant).prefetch_related(
            ...
            Prefetch(
                "clause_entries",
                queryset=AuditReportClause.objects.select_related("asqa_clause"),
            ),
        )
    ...
    def _generate_clause_entries(self, audit_report):
        ...
        for clause in all_clauses:
            entry = AuditReportClause.objects.create(
                audit_report=audit_report, asqa_clause=clause
            )
