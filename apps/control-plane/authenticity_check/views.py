...
class AuthenticityCheckViewSet(viewsets.ModelViewSet):
    ...
    def check_authenticity(self, request, pk=None):
        ...
        submission_content = request.data.get(
            "content", "Sample submission content for analysis."
        )
    ...
    def _detect_plagiarism(self, current_analysis, authenticity_check):
        ...
        results = {"matches_found": 0, "highest_similarity": 0.0, "matches": []}
    ...
    def _verify_metadata(self, analysis):
        ...
        verification = MetadataVerification.objects.create(
            ...
            author_info={"name": "Student Name", "email": "student@example.com"},
    ...
    def _detect_anomalies(self, analysis):
        ...
        return {
            ...
            "anomalies": [
                AnomalyDetectionSerializer(a).data for a in anomalies_created
            ],
        }
    ...
    def integrity_report(self, request, pk=None):
        ...
        report = {
            ...
            "statistics": {
                ...
                "pass_rate": analyses.filter(integrity_status="pass").count()
                / max(analyses.count(), 1)
                * 100,
            },
            "score_distribution": {
            ...
            "average_scores": {
                "plagiarism": analyses.aggregate(Avg("plagiarism_score"))[
                    "plagiarism_score__avg"
                ]
                or 0,
                "metadata_verification": analyses.aggregate(
                    Avg("metadata_verification_score")
                )["metadata_verification_score__avg"]
                or 0,
                "anomaly": analyses.aggregate(Avg("anomaly_score"))[
                    "anomaly_score__avg"
                ]
                or 0,
                "combined_integrity": analyses.aggregate(
                    Avg("combined_integrity_score")
                )["combined_integrity_score__avg"]
                or 0,
            },
