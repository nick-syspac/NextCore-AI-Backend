...
class AuthenticityCheck(models.Model):
    ...
    assessment = models.ForeignKey('assessment_builder.Assessment', on_delete=models.CASCADE, related_name='authenticity_checks')
    ...
    created_by = models.ForeignKey('auth.User', on_delete=models.SET_NULL, null=True, related_name='authenticity_checks_created')
    ...
    def save(self, *args, **kwargs):
        if not self.check_number:
            ...
            last_check = AuthenticityCheck.objects.filter(
...
class SubmissionAnalysis(models.Model):
    ...
    authenticity_check = models.ForeignKey(AuthenticityCheck, on_delete=models.CASCADE, related_name='submission_analyses')
    ...
    # Analysis results
    integrity_status = models.CharField(max_length=20, choices=INTEGRITY_STATUS_CHOICES, default='pass')
    ...
    def save(self, *args, **kwargs):
        if not self.analysis_number:
            date_str = timezone.now().strftime('%Y%m%d')
            last_analysis = SubmissionAnalysis.objects.filter(
    ...
    def calculate_combined_score(self):
        ...
        score -= (100 - self.metadata_verification_score) * 0.2  # 20% weight for metadata
...
class PlagiarismMatch(models.Model):
    ...
    match_type = models.CharField(max_length=20, choices=MATCH_TYPE_CHOICES, default='embedding')
    ...
    def save(self, *args, **kwargs):
        if not self.match_number:
            date_str = timezone.now().strftime('%Y%m%d')
            last_match = PlagiarismMatch.objects.filter(
...
class MetadataVerification(models.Model):
    ...
    submission_analysis = models.ForeignKey(SubmissionAnalysis, on_delete=models.CASCADE, related_name='metadata_verifications')
    ...
    modification_history = models.JSONField(
        default=list,
        help_text="History of file modifications"
    )
    ...
    anomalies_detected = models.JSONField(
        default=list,
        help_text="List of detected metadata anomalies"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.verification_number:
            date_str = timezone.now().strftime('%Y%m%d')
            last_verification = MetadataVerification.objects.filter(
                verification_number__startswith=f'VER-{date_str}-'
            ).order_by('-verification_number').first()
...
class AnomalyDetection(models.Model):
    ...
    submission_analysis = models.ForeignKey(SubmissionAnalysis, on_delete=models.CASCADE, related_name='anomaly_detections')
    ...
    anomaly_data = models.JSONField(
        default=dict,
        help_text="Detailed anomaly data and evidence"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.anomaly_number:
            date_str = timezone.now().strftime('%Y%m%d')
            last_anomaly = AnomalyDetection.objects.filter(
                anomaly_number__startswith=f'ANM-{date_str}-'
            ).order_by('-anomaly_number').first()
        ...
        self.impact_score = severity_multiplier.get(self.severity, 0.5) * self.confidence_score * 100
