...
class RubricViewSet(viewsets.ModelViewSet):
        ...
        try:
            ...
            for i, criterion_data in enumerate(generation_result['criteria'], 1):
                criterion = RubricCriterion.objects.create(
                    ...
                    maps_to_performance_criteria=criterion_data.get('maps_to_performance_criteria', []),
    ...
    def _generate_with_nlp(self, rubric, assessment, task, num_criteria, num_levels, 
                           include_examples, enable_nlp, enable_taxonomy):
        """
...
""".strip()
        ...
        level_definitions = self._get_performance_levels(num_levels, points_per_criterion)
        
        # Generate criteria based on context
        criterion_templates = self._get_criterion_templates(assessment, task, num_criteria)
        ...
        for i, template in enumerate(criterion_templates[:num_criteria]):
            criterion_data = {
                ...
                'maps_to_elements': assessment.elements_covered[:1] if assessment and assessment.elements_covered else [],
                'maps_to_performance_criteria': assessment.performance_criteria_covered[:2] if assessment and assessment.performance_criteria_covered else [],
            ...
            for level_def in level_definitions:
                level_data = {
                    ...
                    'examples': level_def.get('example_template', '') if include_examples else '',
...
""".strip()
    ...
    def _calculate_blooms_distribution(self, num_criteria):
        ...
        for i, level in enumerate(levels):
            ...
            distribution[level] = round((weight / sum(range(1, num_criteria + 1))) * 100, 1)
            ...
            return [
                {
                    ...
                    'indicators': ['Comprehensive coverage', 'Deep understanding', 'Innovation demonstrated'],
                ...
                },
                {
                    ...
                    'indicators': ['Complete coverage', 'Clear understanding', 'Competent application'],
                ...
                },
                {
                    ...
                    'indicators': ['Partial coverage', 'Basic understanding', 'Some gaps'],
                ...
                },
                {
                    ...
                    'indicators': ['Minimal coverage', 'Limited understanding', 'Significant gaps'],
        ...
        templates = [
            {
                ...
                'nlp_keywords': ['knowledge', 'understanding', 'concepts', 'principles'],
            ...
            },
            {
                ...
                'nlp_keywords': ['communication', 'presentation', 'clarity', 'professional'],
    ...
    def dashboard_stats(self, request):
        ...
        stats = {
            ...
            'by_status': dict(rubrics.values('status').annotate(count=Count('id')).values_list('status', 'count')),
            'by_type': dict(rubrics.values('rubric_type').annotate(count=Count('id')).values_list('rubric_type', 'count')),
        ...
        if stats['total_rubrics'] > 0:
            stats['ai_generation_rate'] = round((stats['ai_generated_count'] / stats['total_rubrics']) * 100, 1)
            
            # Calculate average criteria
            total_criteria = sum(r.get_criterion_count() for r in rubrics)
            stats['avg_criteria_per_rubric'] = round(total_criteria / stats['total_rubrics'], 1)
