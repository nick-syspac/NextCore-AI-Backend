...
class Rubric(models.Model):
    ...
    rubric_type = models.CharField(max_length=20, choices=RUBRIC_TYPE_CHOICES, default='analytic')
    ...
    nlp_key_points = models.JSONField(default=list, blank=True, help_text='Extracted key assessment points')
    ...
    # Review workflow
    reviewed_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='reviewed_rubrics')
    reviewed_at = models.DateTimeField(null=True, blank=True)
    approved_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='approved_rubrics')
...
class RubricCriterion(models.Model):
    ...
    nlp_keywords = models.JSONField(default=list, blank=True, help_text='NLP-extracted keywords')
...
class RubricLevel(models.Model):
    ...
    level_name = models.CharField(max_length=50, help_text='e.g., Excellent, Good, Satisfactory')
    ...
    def __str__(self):
        return f"{self.criterion.criterion_number} - {self.level_name} ({self.points} pts)"
...
class RubricGenerationLog(models.Model):
    ...
    rubric = models.ForeignKey(Rubric, on_delete=models.CASCADE, related_name='generation_logs')
