...
class ReadyTechJRConnector(BaseConnector):
    ...
    def test_connection(self) -> tuple[bool, str]:
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/health", headers=self.get_headers(), timeout=10
            )
    ...
    def sync_students(self) -> List[Dict]:
        ...
        response = requests.get(
            ...
            params={
                "modified_since": (
                    self.integration.last_sync_at.isoformat()
    ...
    def sync_units(self) -> List[Dict]:
        ...
        response = requests.get(
            f"{self.base_url}/api/v1/units", headers=self.get_headers()
        )
        response.raise_for_status()
        return response.json()
...
class VETtrakConnector(BaseConnector):
    ...
    def test_connection(self) -> tuple[bool, str]:
        try:
            response = requests.get(
                f"{self.base_url}/api/v1/ping", headers=self.get_headers(), timeout=10
            )
            if response.status_code == 200:
    ...
    def get_clients(self, modified_since: Optional[datetime] = None) -> List[Dict]:
        ...
        response = requests.get(
            f"{self.base_url}/api/v1/clients", headers=self.get_headers(), params=params
        )
        response.raise_for_status()
        return response.json()
    ...
    def get_programs(self) -> List[Dict]:
        ...
        response = requests.get(
            f"{self.base_url}/api/v1/programs", headers=self.get_headers()
        )
        response.raise_for_status()
        return response.json()
...
class eSkilledConnector(BaseConnector):
    ...
    def test_connection(self) -> tuple[bool, str]:
        try:
            response = requests.get(
                f"{self.base_url}/api/status", headers=self.get_headers(), timeout=10
            )
...
class CloudAssessConnector(BaseConnector):
    ...
    def get_assessments(self) -> List[Dict]:
        """Get assessment data"""
        response = requests.get(
            f"{self.base_url}/api/v1/assessments", headers=self.get_headers()
        )
        response.raise_for_status()
        return response.json()
...
class D2LBrightspaceConnector(BaseConnector):
    ...
    def get_org_units(self) -> List[Dict]:
        ...
        response = requests.get(
            f"{self.base_url}/d2l/api/lp/1.0/orgstructure/", headers=self.get_headers()
        )
        response.raise_for_status()
        return response.json()
...
class StripeConnector(BaseConnector):
    ...
    def test_connection(self) -> tuple[bool, str]:
        try:
            response = requests.get(
                f"{self.base_url}/v1/balance", headers=self.get_headers(), timeout=10
            )
    ...
    def create_customer(self, email: str, name: str, metadata: Dict = None) -> Dict:
        ...
        data = {"email": email, "name": name, "metadata": metadata or {}}
        response = requests.post(
            f"{self.base_url}/v1/customers", headers=self.get_headers(), data=data
        )
        response.raise_for_status()
        return response.json()

    def create_payment_intent(
        self, amount: int, currency: str = "aud", customer_id: str = None
    ) -> Dict:
        ...
        response = requests.post(
            f"{self.base_url}/v1/payment_intents", headers=self.get_headers(), data=data
        )
        response.raise_for_status()
        return response.json()
    ...
    def create_subscription(self, customer_id: str, price_id: str) -> Dict:
        ...
        response = requests.post(
            f"{self.base_url}/v1/subscriptions", headers=self.get_headers(), data=data
        )
        response.raise_for_status()
        return response.json()
    ...
    def list_webhooks(self) -> List[Dict]:
        ...
        response = requests.get(
            f"{self.base_url}/v1/webhook_endpoints", headers=self.get_headers()
        )
        response.raise_for_status()
