...
class TASTemplate(models.Model):
    ...
    template_type = models.CharField(
        max_length=50, choices=TEMPLATE_TYPES, default="general"
    )
    ...
    structure = models.JSONField(
        default=dict, help_text="Template structure with sections and GPT-4 prompts"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="tas_templates_created"
    )
...
class TAS(models.Model):
    ...
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="tas_documents"
    )
    title = models.CharField(max_length=300)
    code = models.CharField(
        max_length=50, help_text="Qualification code (e.g., BSB50120)"
    )
    ...
    gpt_model_used = models.CharField(
        max_length=50, blank=True, help_text="GPT model version used"
    )
    gpt_tokens_used = models.IntegerField(default=0)
    generation_time_seconds = models.FloatField(
        default=0.0, help_text="Time taken to generate"
    )

    # Document Content
    content = models.JSONField(
        default=dict, help_text="Full document content including all sections"
    )
    metadata = models.JSONField(
        default=dict, help_text="Additional metadata (units, assessments, etc.)"
    )
    ...
    # Audit
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="tas_created"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if (
            not self.pk
            and TAS.objects.filter(tenant=self.tenant, code=self.code).exists()
        ):
            latest = (
                TAS.objects.filter(tenant=self.tenant, code=self.code)
                .order_by("-version")
                .first()
            )
    ...
    def create_new_version(self, user):
        ...
        TAS.objects.filter(
            tenant=self.tenant, code=self.code, is_current_version=True
    ...
    def get_time_saved(self):
        ...
        if self.gpt_generated and self.generation_time_seconds > 0:
            traditional_time = (
                self.generation_time_seconds * 10
            )  # If GPT saves 90%, traditional = 10x
...
class TASVersion(models.Model):
    ...
    tas = models.ForeignKey(
        TAS, on_delete=models.CASCADE, related_name="version_history"
    )
...
class TASGenerationLog(models.Model):
    ...
    tas = models.ForeignKey(
        TAS, on_delete=models.CASCADE, related_name="generation_logs"
    )
...
class Trainer(models.Model):
    ...
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="trainers"
    )
    ...
    qualifications = models.JSONField(
        default=list, help_text="List of trainer qualifications"
    )
    tae_qualification = models.CharField(
        max_length=100, blank=True, help_text="TAE40116 or equivalent"
    )

    # Industry Currency
    industry_experience_years = models.IntegerField(default=0)
    currency_activities = models.JSONField(
        default=list, help_text="Recent industry currency activities"
    )
    last_currency_date = models.DateField(null=True, blank=True)

    # Scope
    scope_units = models.JSONField(
        default=list, help_text="Unit codes the trainer is qualified to deliver"
    )
    specializations = models.JSONField(
        default=list, help_text="Areas of specialization"
    )
...
class IndustryEngagement(models.Model):
    ...
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="industry_engagements"
    )
    ...
    recommendations = models.JSONField(
        default=list, help_text="Industry recommendations"
    )

    # Links
    related_qualifications = models.JSONField(
        default=list, help_text="Related qualification codes"
    )
    attachments = models.JSONField(
        default=list, help_text="Supporting documentation links"
    )
...
class CourseTAS(models.Model):
    ...
    # Core Identifiers
    tenant = models.ForeignKey(
        Tenant, on_delete=models.CASCADE, related_name="course_tas"
    )
    ...
    # TGA Snapshot
    tga_qualification_snapshot = models.JSONField(
        default=dict, help_text="TGA data snapshot at creation"
    )
    ...
    elective_units = models.JSONField(
        default=list, help_text="Selected elective unit codes and titles"
    )
    total_units = models.IntegerField(default=0)

    # Delivery Model
    cohort_profile = models.TextField(
        help_text="Target cohort description and contextualization"
    )
    delivery_model = models.CharField(
        max_length=100, help_text="Face-to-face, Online, Blended, etc."
    )
    ...
    clusters = models.JSONField(
        default=list, help_text="Unit clusters with sequencing and hours"
    )

    # Resources
    resources = models.JSONField(
        default=list, help_text="Facilities, equipment, materials required"
    )
    facilities = models.ManyToManyField(
        Facility, blank=True, related_name="course_tas_assignments"
    )

    # Assessment Overview
    assessment_overview = models.TextField(
        blank=True, help_text="High-level assessment strategy"
    )

    # Validation & Industry
    validation_plan = models.JSONField(
        default=dict, help_text="Validation schedule and approach"
    )
    industry_engagements = models.ManyToManyField(
        IndustryEngagement, blank=True, related_name="course_tas_links"
    )

    # Compliance & Policy
    policy_links = models.JSONField(
        default=list, help_text="Links to RTO policies (RPL, Assessment, LLN, etc.)"
    )
    ...
    submitted_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="course_tas_submitted",
    )
    reviewed_at = models.DateTimeField(null=True, blank=True)
    reviewed_by = models.ForeignKey(
        User,
        ...
        null=True,
        blank=True,
        related_name="course_tas_reviewed",
    )
    approved_at = models.DateTimeField(null=True, blank=True)
    approved_by = models.ForeignKey(
        User,
        ...
        null=True,
        blank=True,
        related_name="course_tas_approved",
    )
    published_at = models.DateTimeField(null=True, blank=True)

    # Audit
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="course_tas_created"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def __str__(self):
        return (
            f"{self.qualification_code} - {self.qualification_name} (v{self.version})"
        )
...
class UnitTAS(models.Model):
    ...
    course_tas = models.ForeignKey(
        CourseTAS, on_delete=models.CASCADE, related_name="unit_tas_set"
    )
    ...
    unit_type = models.CharField(
        max_length=20, choices=[("core", "Core"), ("elective", "Elective")]
    )
    nominal_hours = models.IntegerField(default=0)

    # TGA Unit Snapshot
    tga_unit_snapshot = models.JSONField(
        default=dict, help_text="Elements, PC, KE, FS from TGA"
    )

    # Delivery Sequence
    cluster_assignment = models.CharField(
        max_length=100, blank=True, help_text="Which cluster/term"
    )
    delivery_sequence = models.JSONField(
        default=dict, help_text="Week-by-week delivery plan"
    )
    prerequisites = models.JSONField(default=list, help_text="Required prior units")

    # Assessment Plan
    assessment_plan = models.JSONField(
        default=list,
        help_text="Assessment plan summary (deprecated - use assessment_tasks relation)",
    )
    mapping_matrix = models.JSONField(
        default=dict, help_text="Elements/PC/KE/FS → tasks → instruments"
    )

    # Resources
    resources = models.JSONField(
        default=list, help_text="Specific resources for this unit"
    )
    facilities = models.ManyToManyField(
        Facility, blank=True, related_name="unit_tas_assignments"
    )

    # Trainers
    trainers = models.ManyToManyField(
        Trainer, blank=True, related_name="unit_tas_assignments"
    )

    # Contextualization
    cohort_context = models.TextField(
        blank=True, help_text="How this unit suits the cohort"
    )
    ...
    # Audit
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="unit_tas_created"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
...
class ComplianceRule(models.Model):
    ...
    rule_logic = models.JSONField(
        default=dict,
        help_text="Rule implementation as JSON (e.g., conditions, thresholds)",
    )
    severity = models.CharField(max_length=20, choices=SEVERITY)

    # ASQA Clause Reference
    asqa_clause = models.CharField(
        max_length=50, blank=True, help_text="e.g., 1.1, 1.13, 2.2"
    )

    # Status
    is_active = models.BooleanField(default=True)
    is_system_rule = models.BooleanField(
        default=False, help_text="System rules cannot be deleted"
    )
...
class ComplianceCheck(models.Model):
    ...
    rule = models.ForeignKey(
        ComplianceRule, on_delete=models.CASCADE, related_name="checks"
    )
    ...
    resolved_by = models.ForeignKey(
        ...
        null=True,
        blank=True,
        related_name="compliance_resolutions",
    )
...
class AssessmentTask(models.Model):
    ...
    unit_tas = models.ForeignKey(
        UnitTAS, on_delete=models.CASCADE, related_name="assessment_tasks"
    )
    ...
    performance_criteria_covered = models.JSONField(
        default=list, help_text="PC codes covered"
    )
    knowledge_evidence_covered = models.JSONField(
        default=list, help_text="KE items covered"
    )
    foundation_skills_covered = models.JSONField(
        default=list, help_text="FS items covered"
    )
    ...
    instruments = models.JSONField(
        default=list, help_text="Assessment instruments/tools used"
    )
...
class ImmutableSnapshot(models.Model):
    ...
    course_tas = models.ForeignKey(
        CourseTAS, on_delete=models.CASCADE, related_name="snapshots"
    )

    # Snapshot Content
    snapshot_data = models.JSONField(
        help_text="Complete frozen state of CourseTAS and all UnitTAS"
    )
    version = models.IntegerField()

    # Integrity
    checksum = models.CharField(
        max_length=64, help_text="SHA-256 hash of snapshot data"
    )

    # Metadata
    snapshot_reason = models.CharField(
        max_length=100, help_text="e.g., 'Initial Approval', 'Version Update'"
    )
    evidence_pack_links = models.JSONField(
        default=list, help_text="Links to supporting evidence"
    )
    ...
    # Auditor Access
    auditor_link_token = models.CharField(
        max_length=100, blank=True, help_text="Secure token for auditor access"
    )
...
class EvidencePack(models.Model):
    ...
    course_tas = models.ForeignKey(
        CourseTAS, on_delete=models.CASCADE, related_name="evidence_packs"
    )
    ...
    # Pack Contents
    industry_engagement_links = models.JSONField(
        default=list, help_text="Links to IndustryEngagement records"
    )
    validation_evidence = models.JSONField(
        default=list, help_text="Validation meeting notes, reports"
    )
    policy_documents = models.JSONField(
        default=list, help_text="Linked RTO policy documents"
    )
    trainer_credentials = models.JSONField(
        default=list, help_text="Trainer qualifications and currency evidence"
    )
    facility_documentation = models.JSONField(
        default=list, help_text="Facility and resource evidence"
    )
    change_log = models.JSONField(
        default=list, help_text="History of changes and justifications"
    )
    ...
    export_file_path = models.CharField(
        max_length=500, blank=True, help_text="Path to exported evidence bundle"
    )
