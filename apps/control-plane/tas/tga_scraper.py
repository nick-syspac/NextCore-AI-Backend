...
class TrainingGovAuScraper:
    ...
    def fetch_qualification_units(self, qualification_code: str) -> Optional[Dict]:
        ...
        try:
            ...
            if not title_elem:
                logger.error(
                    f"❌ Could not find qualification title for {qualification_code}"
                )
                return None
            ...
            logger.info(
                f"✅ Successfully fetched {len(groupings)} groupings with {sum(len(g['units']) for g in groupings)} total units"
            )
            return result
    ...
    def _extract_packaging_rules(self, soup: BeautifulSoup) -> str:
        ...
        try:
            ...
            packaging_section = soup.find(
                "h2", string=re.compile(r"Packaging Rules", re.I)
            )
            if packaging_section:
                # Get the next sibling element (usually a div or p)
                rules_elem = packaging_section.find_next_sibling()
                if rules_elem:
                    return rules_elem.get_text(strip=True, separator=" ")[
                        :500
                    ]  # Limit to 500 chars
    ...
    def _extract_units(self, soup: BeautifulSoup, qual_code: str) -> List[Dict]:
        ...
        try:
            ...
            core_section = soup.find(
                ["h2", "h3", "h4"], string=re.compile(r"Core\s+Unit", re.I)
            )
            if core_section:
            ...
            elective_section = soup.find(
                ["h2", "h3", "h4"], string=re.compile(r"Elective\s+Unit", re.I)
            )
            if elective_section:
            ...
            group_sections = soup.find_all(
                ["h2", "h3", "h4"],
                string=re.compile(r"Group\s+[A-Z]|Stream|Major|Specialisation", re.I),
            )
            ...
            for match in unit_pattern.finditer(text):
                ...
                if code not in seen_codes:
                    ...
                    for elem in elements:
                        ...
                        units.append(
                            {"code": code, "title": f"Unit {code}", "type": "core"}
                        )
