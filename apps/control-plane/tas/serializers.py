...
class TASTemplateSerializer(serializers.ModelSerializer):
    ...
    aqf_level_display = serializers.CharField(
        source="get_aqf_level_display", read_only=True
    )
...
class TASGenerateRequestSerializer(serializers.Serializer):
    ...
    delivery_mode = serializers.CharField(
        max_length=100, required=False, default="Face-to-face"
    )
...
class TASUpdateSerializer(serializers.Serializer):
    ...
    create_version = serializers.BooleanField(
        default=False, help_text="Create a new version instead of updating current one"
    )
    change_summary = serializers.CharField(
        required=False,
        allow_blank=True,
