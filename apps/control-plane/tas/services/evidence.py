...
from ..models import (
    CourseTAS, UnitTAS, ImmutableSnapshot, EvidencePack,
    IndustryEngagement
)
...
class EvidenceSnapshotService:
    ...
    @classmethod
    def create_snapshot(cls, course_tas: CourseTAS, reason: str, user) -> ImmutableSnapshot:
        """
    ...
    def _gather_snapshot_data(cls, course_tas: CourseTAS) -> Dict:
        ...
        course_data = {
            ...
            'version': course_tas.version,
            'approved_at': course_tas.approved_at.isoformat() if course_tas.approved_at else None,
            'approved_by': course_tas.approved_by.username if course_tas.approved_by else None,
        }
    ...
    def verify_snapshot_integrity(cls, snapshot_id: int) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'sealed_by': snapshot.sealed_by.username if snapshot.sealed_by else None,
            }
    ...
    @classmethod
    def create_evidence_pack(cls, course_tas: CourseTAS, snapshot: ImmutableSnapshot, user) -> EvidencePack:
        """
            ...
            for unit_tas in course_tas.unit_tas_set.all():
                for trainer in unit_tas.trainers.all():
                    trainer_credentials.append({
                        'unit_code': unit_tas.unit_code,
                        'trainer': trainer.user.get_full_name() or trainer.user.username,
                        'qualifications': trainer.qualifications,
                        'tae_qualification': trainer.tae_qualification,
                        'currency_date': trainer.last_currency_date.isoformat() if trainer.last_currency_date else None,
                    })
    ...
    def generate_auditor_link(cls, snapshot_id: int, expiry_days: int = 365) -> str:
        ...
        try:
            ...
            if not snapshot.auditor_link_token or snapshot.auditor_link_expires_at < timezone.now():
                snapshot.auditor_link_token = secrets.token_urlsafe(32)
                snapshot.auditor_link_expires_at = timezone.now() + timedelta(days=expiry_days)
                snapshot.save()
    ...
    def get_snapshot_by_token(cls, token: str) -> ImmutableSnapshot:
        ...
        try:
            snapshot = ImmutableSnapshot.objects.get(
                auditor_link_token=token,
                auditor_link_expires_at__gt=timezone.now()
            )
            return snapshot
        except ImmutableSnapshot.DoesNotExist:
            return None
