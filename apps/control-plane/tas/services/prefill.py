...
class PrefillService:
    ...
    def get_available_trainers(
        cls, tenant, unit_codes: List[str] = None
    ) -> List[Trainer]:
        """
    ...
    def get_available_facilities(
        cls, tenant, facility_types: List[str] = None
    ) -> List[Facility]:
        """
    ...
    @classmethod
    def get_recent_industry_engagements(
        cls, tenant, qualification_code: str = None
    ) -> List[IndustryEngagement]:
        """
        ...
        """
        queryset = IndustryEngagement.objects.filter(tenant=tenant).order_by(
            "-engagement_date"
        )
    ...
    def prefill_course_tas(cls, tenant, qualification_code: str) -> Dict:
        ...
        prefill_data = {
            ...
            "recent_industry_engagements": cls.get_recent_industry_engagements(
                tenant, qualification_code
            ),
        }
    ...
    @classmethod
    def prefill_unit_tas(
        cls, tenant, unit_code: str, course_tas: CourseTAS = None
    ) -> Dict:
        """
