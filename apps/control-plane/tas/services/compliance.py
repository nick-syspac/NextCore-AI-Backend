...
from ..models import CourseTAS, UnitTAS, ComplianceRule, ComplianceCheck, Trainer
...
class ComplianceRAGEngine:
    ...
    def run_all_checks(cls, course_tas: CourseTAS, user=None) -> Dict:
        ...
        rules = ComplianceRule.objects.filter(
            models_module.Q(tenant=course_tas.tenant)
            | models_module.Q(is_system_rule=True),
            is_active=True,
        )
    ...
    def run_unit_checks(cls, unit_tas: UnitTAS, user=None) -> Dict:
        ...
        unit_rules = ComplianceRule.objects.filter(
            models_module.Q(tenant=unit_tas.course_tas.tenant)
            | models_module.Q(is_system_rule=True),
            is_active=True,
    ...
    @classmethod
    def _check_packaging_rules(
        cls, rule: ComplianceRule, course_tas: CourseTAS
    ) -> Dict:
        """
        ...
        packaging_rules = course_tas.tga_qualification_snapshot.get(
            "packaging_rules", {}
        )
        ...
        if elective_count < required_electives:
            issues.append(
                f"Insufficient elective units: {elective_count}/{required_electives}"
            )
    ...
    @classmethod
    def _check_course_trainer_scope(
        cls, rule: ComplianceRule, course_tas: CourseTAS
    ) -> Dict:
        """
    ...
    @classmethod
    def _check_unit_facility_adequacy(
        cls, rule: ComplianceRule, unit_tas: UnitTAS
    ) -> Dict:
    ...
    def _check_hours_validation(
        ...
        # Allow 10% variance
        variance = (
            abs(total_hours - expected_hours) / expected_hours
            if expected_hours > 0
            else 0
        )
    ...
    def _check_clustering_sanity(
        ...
        # Basic sanity checks
        all_units = [
            u["code"] for u in course_tas.core_units + course_tas.elective_units
        ]
        clustered_units = []
