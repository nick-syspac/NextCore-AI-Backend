...
urlpatterns = [
    ...
    path(
        "tenants/<str:tenant_slug>/policy-comparator/",
        include("policy_comparator.urls"),
    ),
    path("tenants/<str:tenant_slug>/audit-assistant/", include("audit_assistant.urls")),
    path(
        "tenants/<str:tenant_slug>/continuous-improvement/",
        include("continuous_improvement.urls"),
    ),
    path(
        "tenants/<str:tenant_slug>/funding-eligibility/",
        include("funding_eligibility.urls"),
    ),
    path(
        "tenants/<str:tenant_slug>/assessment-builder/",
        include("assessment_builder.urls"),
    ),
    path(
        "tenants/<str:tenant_slug>/rubric-generator/", include("rubric_generator.urls")
    ),
    path("tenants/<str:tenant_slug>/auto-marker/", include("auto_marker.urls")),
    path(
        "tenants/<str:tenant_slug>/feedback-assistant/",
        include("feedback_assistant.urls"),
    ),
    ...
    path(
        "tenants/<str:tenant_slug>/authenticity-check/",
        include("authenticity_check.urls"),
    ),
    path("tenants/<str:tenant_slug>/risk-engine/", include("risk_engine.urls")),
    path(
        "tenants/<str:tenant_slug>/adaptive-pathway/", include("adaptive_pathway.urls")
    ),
    path(
        "tenants/<str:tenant_slug>/engagement-heatmap/",
        include("engagement_heatmap.urls"),
    ),
    path("tenants/<str:tenant_slug>/study-coach/", include("study_coach.urls")),
    path(
        "tenants/<str:tenant_slug>/intervention-tracker/",
        include("intervention_tracker.urls"),
    ),
