        ...
        try:
            ...
            for i, task_data in enumerate(generation_result['tasks'], 1):
                task = AssessmentTask.objects.create(
                    ...
                    maps_to_performance_criteria=task_data.get('maps_to_performance_criteria', []),
                    question_count=1,
...
"""
        ...
        for i in range(num_tasks):
            ...
            question = self._generate_question(unit_title, primary_verb, blooms_level, i + 1)
            ...
            tasks.append({
                ...
                'maps_to_elements': assessment.elements_covered[:1] if assessment.elements_covered else [],
                'maps_to_performance_criteria': assessment.performance_criteria_covered[:2] if assessment.performance_criteria_covered else [],
                'estimated_time': 10 if task_type in ['short_answer', 'multiple_choice'] else 20,
            })
    ...
    def _get_blooms_verbs(self, level):
        ...
        """
        blooms_verb_map = {
            'remember': ['list', 'define', 'describe', 'identify', 'name', 'recall', 'recognize'],
    ...
    def _generate_question(self, unit_title, verb, blooms_level, question_num):
        ...
        base_question = templates.get(blooms_level, f"{verb.capitalize()} aspects of {unit_title}.")
    ...
    def dashboard_stats(self, request):
        ...
        total_assessments_with_blooms = sum(1 for a in assessments if a.blooms_distribution)
        ...
        stats = {
            ...
            'by_status': dict(assessments.values('status').annotate(count=Count('id')).values_list('status', 'count')),
            'by_type': dict(assessments.values('assessment_type').annotate(count=Count('id')).values_list('assessment_type', 'count')),
            'ai_generated_count': assessments.filter(ai_generated=True).count(),
            'ai_generation_rate': 0.0,
            'avg_compliance_score': assessments.aggregate(avg=Avg('compliance_score'))['avg'] or 0,
        ...
        if stats['total_assessments'] > 0:
            stats['ai_generation_rate'] = round((stats['ai_generated_count'] / stats['total_assessments']) * 100, 1)
            
            # Calculate average tasks
            total_tasks = sum(a.get_task_count() for a in assessments)
            stats['avg_tasks_per_assessment'] = round(total_tasks / stats['total_assessments'], 1)
