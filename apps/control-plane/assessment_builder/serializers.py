...
class AssessmentSerializer(serializers.ModelSerializer):
    ...
    total_questions = serializers.IntegerField(
        source="get_total_questions", read_only=True
    )
...
class AssessmentGenerationRequestSerializer(serializers.Serializer):
    ...
    training_package = serializers.CharField(
    ...
    elements = serializers.ListField(
        child=serializers.CharField(), required=False, help_text="List of unit elements"
    )
    ...
    number_of_tasks = serializers.IntegerField(
        min_value=1, max_value=50, default=10, help_text="Number of tasks to generate"
    )
    include_context = serializers.BooleanField(
        default=True, help_text="Include realistic context/scenarios"
    )
