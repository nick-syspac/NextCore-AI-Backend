...
class Assessment(models.Model):
    ...
    unit_code = models.CharField(
        max_length=50, help_text="Training package unit code (e.g., BSBWHS211)"
    )
    unit_title = models.CharField(max_length=300)
    training_package = models.CharField(
        max_length=100, blank=True, help_text="e.g., BSB - Business Services"
    )
    unit_release = models.CharField(
        max_length=20, blank=True, help_text="Release number"
    )
    ...
    instructions = models.TextField(
        blank=True, help_text="AI-generated assessment instructions"
    )
    context = models.TextField(blank=True, help_text="Assessment context and scenario")
    conditions = models.TextField(
        blank=True, help_text="Assessment conditions (equipment, resources, etc.)"
    )
    ...
    dominant_blooms_level = models.CharField(
        max_length=20, blank=True, help_text="Most prominent Bloom's level"
    )
    ...
    elements_covered = models.JSONField(
        default=list, blank=True, help_text="List of unit elements covered"
    )
    performance_criteria_covered = models.JSONField(
        default=list, blank=True, help_text="List of performance criteria covered"
    )
    knowledge_evidence_covered = models.JSONField(
        default=list, blank=True, help_text="List of knowledge evidence items covered"
    )
    performance_evidence_covered = models.JSONField(
        default=list, blank=True, help_text="List of performance evidence items covered"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="created_assessments"
    )
...
class AssessmentTask(models.Model):
    ...
    assessment = models.ForeignKey(
        Assessment, on_delete=models.CASCADE, related_name="tasks"
    )
    ...
    ai_rationale = models.TextField(
        blank=True, help_text="AI explanation of why this task was generated"
    )

    # Bloom's Taxonomy
    blooms_level = models.CharField(
        max_length=20, blank=True, help_text="Primary Bloom's taxonomy level"
    )
    ...
    maps_to_elements = models.JSONField(
        default=list, blank=True, help_text="Unit elements this task addresses"
    )
    maps_to_performance_criteria = models.JSONField(
        default=list, blank=True, help_text="Performance criteria this task addresses"
    )
    maps_to_knowledge_evidence = models.JSONField(
        default=list, blank=True, help_text="Knowledge evidence this task addresses"
    )

    # Task metadata
    question_count = models.IntegerField(
        default=1, help_text="Number of sub-questions (for multi-part tasks)"
    )
    estimated_time_minutes = models.IntegerField(
        null=True, blank=True, help_text="Estimated completion time in minutes"
    )

    # Marking
    marks_available = models.IntegerField(
        null=True, blank=True, help_text="Total marks for this task"
    )
...
class AssessmentCriteria(models.Model):
    ...
    assessment = models.ForeignKey(
        Assessment, on_delete=models.CASCADE, related_name="criteria"
    )
    ...
    satisfactory_evidence = models.TextField(
        blank=True, help_text="What constitutes satisfactory performance"
    )
    not_satisfactory_evidence = models.TextField(
        blank=True, help_text="What would be unsatisfactory"
    )
...
class AssessmentGenerationLog(models.Model):
    ...
    assessment = models.ForeignKey(
        Assessment, on_delete=models.CASCADE, related_name="generation_logs"
    )
