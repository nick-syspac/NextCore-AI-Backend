...
from .models import (
    RiskAssessment, RiskFactor, StudentEngagementMetric,
    SentimentAnalysis, InterventionAction
)
...
class RiskAssessmentAdmin(admin.ModelAdmin):
    list_display = [
        'assessment_number', 'student_name', 'risk_level', 'dropout_probability',
        'risk_score', 'confidence', 'status', 'alert_triggered', 'assessment_date'
    ]
    list_filter = ['risk_level', 'status', 'alert_triggered', 'alert_acknowledged', 'assessment_date']
    search_fields = ['assessment_number', 'student_id', 'student_name']
    readonly_fields = ['assessment_number', 'risk_level', 'assessment_date', 'created_at', 'last_updated']
    ...
    fieldsets = (
        ...
        ('Risk Scores', {
            'fields': (
                'dropout_probability', 'risk_level', 'risk_score', 'confidence',
        ...
        ('Status & Alerts', {
            'fields': (
                'status', 'alert_triggered', 'alert_acknowledged',
                'alert_acknowledged_by', 'alert_acknowledged_at'
            )
        }),
        ('Intervention', {
            'fields': ('intervention_assigned', 'intervention_notes')
        }),
        ('Metadata', {
            'fields': ('assessment_date', 'last_updated', 'created_by', 'created_at'),
            'classes': ('collapse',)
        }),
    )
...
class RiskFactorAdmin(admin.ModelAdmin):
    list_display = [
        'factor_number', 'assessment', 'factor_type', 'factor_name',
        'severity', 'contribution', 'threshold_exceeded', 'trend'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('factor_number', 'assessment', 'factor_type', 'factor_name', 'description')
        }),
        ('Metrics', {
            'fields': ('weight', 'contribution', 'severity', 'trend')
        }),
        ('Data Points', {
...
class StudentEngagementMetricAdmin(admin.ModelAdmin):
    list_display = [
        'metric_number', 'student_name', 'overall_engagement_score',
        'login_frequency', 'assignment_submission_rate', 'days_inactive', 'measurement_date'
    ]
    ...
    fieldsets = (
        ...
        ('Engagement Metrics', {
            'fields': (
                'login_frequency', 'time_on_platform', 'assignment_submission_rate',
                'forum_participation', 'peer_interaction_score'
            )
        }),
        ('Activity Patterns', {
        ...
        ('Overall Score', {
            'fields': ('overall_engagement_score',)
        }),
        ('Metadata', {
            'fields': ('measurement_date', 'created_at'),
            'classes': ('collapse',)
        }),
    )
...
class SentimentAnalysisAdmin(admin.ModelAdmin):
    list_display = [
        'analysis_number', 'assessment', 'source_type', 'sentiment_label',
    ...
    fieldsets = (
        ...
        ('Emotion Detection', {
            'fields': (
                'frustration_detected', 'stress_detected',
                'confusion_detected', 'disengagement_detected'
            )
        }),
        ('Keywords & Patterns', {
            'fields': ('negative_keywords', 'risk_indicators')
        }),
        ('Metadata', {
...
class InterventionActionAdmin(admin.ModelAdmin):
    list_display = [
        'action_number', 'assessment', 'action_type', 'priority',
        'status', 'scheduled_date', 'assigned_to'
    ]
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('action_number', 'assessment', 'action_type', 'description', 'priority')
        }),
        ('Scheduling', {
            'fields': ('scheduled_date', 'completed_date', 'status', 'assigned_to')
        }),
        ('Outcomes', {
            'fields': ('outcome_notes', 'effectiveness_rating')
        }),
        ('Metadata', {
