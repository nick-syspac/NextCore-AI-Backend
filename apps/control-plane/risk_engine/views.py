...
from .serializers import (
    RiskAssessmentSerializer, RiskAssessmentListSerializer,
    RiskFactorSerializer, StudentEngagementMetricSerializer,
    SentimentAnalysisSerializer, InterventionActionSerializer
)
...
class RiskAssessmentViewSet(viewsets.ModelViewSet):
    ...
    def predict_risk(self, request):
        ...
        risk_factors = self._generate_risk_factors(
            assessment, engagement_score, performance_score, 
            attendance_score, sentiment_score
        )
    ...
    def _logistic_regression_model(self, engagement, performance, attendance, sentiment):
        """
    ...
    def _calculate_model_confidence(self, engagement, performance, attendance, sentiment):
    ...
    def _generate_risk_factors(self, assessment, engagement, performance, attendance, sentiment):
        """Generate detailed risk factors for the assessment"""
        risk_factors = []
        
        # Engagement factor
        if engagement < 60:
            severity = 'critical' if engagement < 30 else 'high' if engagement < 45 else 'medium'
        ...
        # Performance factor
        if performance < 65:
            severity = 'critical' if performance < 40 else 'high' if performance < 50 else 'medium'
        ...
        # Attendance factor
        if attendance < 75:
            severity = 'critical' if attendance < 50 else 'high' if attendance < 60 else 'medium'
        ...
        # Sentiment factor
        if sentiment < 0:
            severity = 'critical' if sentiment < -0.5 else 'high' if sentiment < -0.3 else 'medium'
    ...
    def _generate_sentiment_analyses(self, assessment, sentiment_data):
        ...
        for text_item in texts[:3]:  # Limit to 3 samples
            ...
            SentimentAnalysis.objects.create(
                ...
                negative_keywords=['difficult', 'confused', 'struggling'] if score < 0 else [],
    ...
    def alerts_dashboard(self, request):
        ...
        risk_breakdown = RiskAssessment.objects.values('risk_level').annotate(
            count=Count('id')
        ).order_by('risk_level')
        ...
        avg_dropout_prob = RiskAssessment.objects.aggregate(
            avg=Avg('dropout_probability')
        )['avg'] or 0.0
        
        return Response({
            'active_alerts': active_alerts,
            'risk_breakdown': list(risk_breakdown),
            'high_risk_students': RiskAssessmentListSerializer(high_risk, many=True).data,
