...
from .serializers import (
    EvidenceMappingSerializer, SubmissionEvidenceSerializer, SubmissionEvidenceDetailSerializer,
    CriteriaTagSerializer, EvidenceAuditSerializer, EmbeddingSearchSerializer,
    TagEvidenceRequestSerializer, ExtractTextRequestSerializer, SearchEmbeddingsRequestSerializer
)
...
class EvidenceMappingViewSet(viewsets.ModelViewSet):
    ...
    def coverage_report(self, request, pk=None):
        ...
        criteria_with_evidence = CriteriaTag.objects.filter(
            evidence__mapping=mapping
        ).values('criterion_id', 'criterion_name').annotate(
            tag_count=models.Count('id')
        ).order_by('criterion_id')
        
        # Calculate statistics
        total_tags = CriteriaTag.objects.filter(evidence__mapping=mapping).count()
        validated_tags = CriteriaTag.objects.filter(evidence__mapping=mapping, is_validated=True).count()
        ...
        coverage_data = {
            ...
            'validation_rate': (validated_tags / total_tags * 100) if total_tags > 0 else 0,
...
class SubmissionEvidenceViewSet(viewsets.ModelViewSet):
    ...
    def extract_text(self, request, pk=None):
        ...
        if generate_embedding and not submission.text_embedding:
            ...
            submission.text_embedding = [random.random() for _ in range(embedding_dimension)]
    ...
    def tag_evidence(self, request, pk=None):
        ...
        context_before = submission.extracted_text[max(0, start_pos - context_size):start_pos]
        context_after = submission.extracted_text[end_pos:min(len(submission.extracted_text), end_pos + context_size)]
        
        # Extract keywords from tagged text
        tagged_words = data['tagged_text'].lower().split()
        common_words = {'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by'}
        keywords = [word for word in tagged_words if len(word) > 3 and word not in common_words][:10]
            ...
            if submission.text_embedding:
                ...
                if similarity >= min_similarity:
                    results.append({
                        ...
                        'text_preview': submission.extracted_text[:200] + '...' if len(submission.extracted_text) > 200 else submission.extracted_text,
