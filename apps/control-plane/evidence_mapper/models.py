...
class EvidenceMapping(models.Model):
    ...
    mapping_number = models.CharField(
        max_length=50, unique=True, blank=True, editable=False
    )
    name = models.CharField(max_length=200)
    description = models.TextField(blank=True)
    assessment_type = models.CharField(
        max_length=50, choices=ASSESSMENT_TYPE_CHOICES, default="written"
    )
    ...
    # Configuration
    auto_extract_text = models.BooleanField(
        default=True, help_text="Automatically extract text from submissions"
    )
    generate_embeddings = models.BooleanField(
        default=True, help_text="Generate embeddings for semantic search"
    )
    require_evidence_per_criterion = models.BooleanField(default=True)
    min_evidence_length = models.IntegerField(
        default=50, help_text="Minimum characters for valid evidence"
    )
    ...
    status = models.CharField(
        max_length=20, choices=MAPPING_STATUS_CHOICES, default="active"
    )
    ...
    def calculate_coverage(self):
        ...
        criteria_with_evidence = (
            CriteriaTag.objects.filter(evidence__mapping=self)
            .values("criterion_id")
            .distinct()
            .count()
        )
...
class SubmissionEvidence(models.Model):
    ...
    mapping = models.ForeignKey(
        EvidenceMapping, on_delete=models.CASCADE, related_name="submissions"
    )
    ...
    submission_type = models.CharField(
        max_length=50, blank=True, help_text="e.g., PDF, DOCX, Video"
    )
    ...
    extracted_text = models.TextField(
        blank=True, help_text="Full text extracted from submission"
    )
    ...
    extraction_method = models.CharField(
        max_length=100, blank=True, help_text="e.g., OCR, PDF parser, Speech-to-text"
    )

    # Embeddings (stored as JSON array)
    text_embedding = models.JSONField(
        default=list, help_text="Vector embedding for semantic search"
    )
    embedding_model = models.CharField(
        max_length=100, blank=True, help_text="e.g., sentence-transformers"
    )
    ...
    criteria_covered = models.JSONField(
        default=list, help_text="List of criterion IDs with evidence"
    )
...
class CriteriaTag(models.Model):
    ...
    evidence = models.ForeignKey(
        SubmissionEvidence, on_delete=models.CASCADE, related_name="tags"
    )

    # Criteria linkage
    criterion_id = models.CharField(
        max_length=100, help_text="Reference to assessment criterion"
    )
    ...
    tagged_text = models.TextField(
        help_text="Excerpt from submission linked to criterion"
    )
    text_start_position = models.IntegerField(
        help_text="Character position in original text"
    )
    text_end_position = models.IntegerField()

    # Context
    context_before = models.TextField(
        blank=True, help_text="Text before tagged excerpt"
    )
    context_after = models.TextField(blank=True, help_text="Text after tagged excerpt")

    # Tag details
    tag_type = models.CharField(
        max_length=20, choices=TAG_TYPE_CHOICES, default="direct"
    )
    confidence_level = models.CharField(
        max_length=20, choices=CONFIDENCE_LEVEL_CHOICES, default="manual"
    )
    ...
    keywords = models.JSONField(
        default=list, help_text="Extracted keywords from tagged text"
    )
...
class EvidenceAudit(models.Model):
    ...
    mapping = models.ForeignKey(
        EvidenceMapping, on_delete=models.CASCADE, related_name="audit_logs"
    )
    ...
    action_data = models.JSONField(
        default=dict, help_text="Detailed data about the action performed"
    )

    # Changes tracking
    changes_made = models.JSONField(
        default=dict, help_text="Before/after values for modifications"
    )
...
class EmbeddingSearch(models.Model):
    ...
    mapping = models.ForeignKey(
        EvidenceMapping, on_delete=models.CASCADE, related_name="searches"
    )
    ...
    query_embedding = models.JSONField(
        default=list, help_text="Vector embedding of query"
    )

    # Filters applied
    filter_criteria = models.JSONField(
        default=dict, help_text="Additional filters (student, date range, etc.)"
    )

    # Results
    results_count = models.IntegerField(default=0)
    top_results = models.JSONField(
        default=list, help_text="Top matching results with scores"
    )

    # Performance
    search_time_ms = models.IntegerField(
        null=True, blank=True, help_text="Search execution time"
    )
